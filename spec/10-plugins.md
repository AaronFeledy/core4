...
- Plugin command metadata is read from the validated plugin command cache during router bootstrap. Plugin command implementation modules are imported only after command resolution and runtime bootstrap reach the command's declared level. The native single dispatcher resolves executable built-ins from `builtInCommandEntries`; the build-embedded command-registry manifest is its metadata projection. Composition with validated plugin-command cache metadata is owned by US-534 (§8, §12.1).
