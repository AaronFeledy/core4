...
- Plugin command metadata is read from the validated plugin command cache during router bootstrap. Plugin command implementation modules are imported only after command resolution and runtime bootstrap reach the command's declared level. The native single-dispatcher registry (§8, §12.1) combines built-ins from the build-embedded command-registry manifest with validated external plugin-command metadata from the separate plugin-command cache.
