...
- The validated `plugin-command` cache (§12.1) is the **sole metadata owner** for every plugin command, bundled and external. Plugin discovery merges sources in `bundled` → `user` → `app` order; a later source replaces an earlier plugin with the same manifest name before cache compilation. Across the resulting manifest order, the first plugin declaring a command id owns that id, and the compiled command index is sorted by canonical id. Bundled metadata is not duplicated into the build-embedded command-registry manifest: that artifact remains the built-in-only projection of `builtInCommandEntries`.
- Plugin-command cache writes and freshness reads use the same resolved manifest set and compare the Lando version, manifest fingerprint, plugin-list fingerprint, and per-plugin command ids. Plugin install, remove, or update; explicit plugin-cache invalidation; `--clear`; or a regenerated bundled ship list invalidates that ownership view. Removing a command-contributing bundled plugin from `core/build.config.ts`, regenerating `core/src/plugins/generated/bundled.ts`, and rewriting the cache therefore removes its command metadata without a hand edit under `core/src/`.
- Router consumption of `plugin-command` metadata and executable plugin-command dispatch are deferred until a public command implementation contract exists. The current `PluginManifest.contributes.commands` entries are only `ContributionRef` ids, and `LandoPluginModule` exposes no command implementation loader; the cache proves metadata ownership and freshness but MUST NOT be treated as an executable registry. The native dispatcher therefore resolves executable built-ins from `builtInCommandEntries` today.
