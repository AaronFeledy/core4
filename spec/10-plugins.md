...
- The validated `plugin-command` cache (§12.1) is the **sole metadata owner** for every plugin command, bundled and external. Plugin discovery follows the complete §9.3 order — bundled → system → user → app-local `pluginDirs:` → explicit Landofile `plugins:` → experimental — and a later source replaces an earlier plugin with the same manifest name before cache compilation. Duplicate command ids are deterministically deduplicated by retaining their first occurrence in the resulting manifest order, and the compiled command index is sorted by canonical id. Bundled metadata is not duplicated into the build-embedded command-registry manifest: that artifact remains the built-in-only projection of `builtInCommandEntries`.
