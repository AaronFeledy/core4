...
Depends on: **Alpha 1 PRD-06** (the `@lando/core/testing` surface and `TestRuntimeProvider` must already exist).
...
- `core/test/library/**` and `@lando/core/testing` — existing testing surface produced by Alpha 1 PRD-06.
...
- Reject Alpha-3-only frontmatter keys and Alpha-3-only components at schema time with remediation pointing at the deferred-features list in PRD-A2-00.

## User Stories

### US-057: Publish Alpha 2 `GuideFrontmatter` schema

**Description:** As a guide author, I can validate my MDX frontmatter against `GuideFrontmatter` and receive a typed remediation when I use a Alpha-3-only key.
...
- [ ] Accepted keys: `id` (kebab-case, required), `defaultLayer` (only `"scenario"` accepted in Alpha 2; `"e2e"` is schema-rejected with a Alpha 3 remediation referencing §19.11), `provider` (only `"test"` accepted in Alpha 2), `timeout` (positive integer ms, default 60000), `platforms`, `tags`, `skip` (`{ reason, until? }`), and `deprecated` (`DeprecationNotice`, schema-only — no runtime propagation in Alpha 2).
- [ ] Schema-rejected keys with explicit remediation: `tabs`, `axes`, `variants`. Each rejection includes a tagged `NotImplementedError` payload citing §19.16 and the Alpha 3 phase per the PRD-A2-00 deferred list.
- [ ] Round-trips through the §13.2 schema gate (Alpha 1 PRD-07 gate) — encode/decode/JSON Schema all stable.
- [ ] Tests cover happy-path acceptance, each Alpha-3-only key rejection, and the `id` kebab-case rule.
...
- [ ] `ScenarioProps` accepts `id` (required), `render` (default `true`), `reason` (required when `render === false`, ≥ 8 chars), `tags`, `layer` (Alpha 2 accepts `"scenario"` only; `"e2e"` rejected with Alpha 3 remediation).
- [ ] `RunProps` accepts exactly one of `command` (CLI) or `shell` (Bun Shell). `runtime` and `tooling` variants are schema-rejected with §19.14 / Alpha 3 remediation.
- [ ] `VerifyProps` accepts exactly one of `event`, `command`, `file`, `errorTag`. `tooling` and `runtime` variants are schema-rejected with Alpha 3 remediation. The `expect` value validates against an Alpha 2 `MatcherSchema` subset: scalar deep-equal, partial-object match, plus `regex`, `schema`, `anyOf`, `not` operators. `exact`, `allOf`, `oneOf` are Alpha 3+.
...
- [ ] Alpha 2 fields: `guideId`, `scenarioId`, `testDir` (created under `os.tmpdir()` at scope acquire, removed at finalize unless `KEEP_SCENARIO_DIRS=1`), `runtime` (a `TestRuntime` from Alpha 1 PRD-06), `vars` (`Map<string, { value: string; display?: string }>`), `runCli`, `events`, `transcript` (write-only stub — full transcript surface is in PRD-A2-04), `fixtures`.
...
- [ ] `runCli` invokes the CLI via the existing Alpha 1 PRD-06 in-process runner (not a subprocess). Captured `stdout`, `stderr`, `exitCode`, and the published `events` stream are recorded in the active transcript stub.
...
**Acceptance Criteria:**
- [ ] A central `assertAlpha 2Component(componentName, hostPath)` helper in `@lando/sdk/docs/components` throws `NotImplementedError(commandId: "guide.component.hidden", specSection: "§19.3")` whose remediation reads (verbatim, including the phase name): "Move this coverage into a colocated `<Scenario render={false}>` per §19.9. `<Hidden>` ships in Phase 3 Alpha 3 — see `spec/ROADMAP.md`."
- [ ] The helper is also wired to reject any non-Alpha 1-2 component name (`<Inspect>`, `<Tabs>`, `<Tab>`, `<Inline>`, `<Skip>`) with a one-line remediation citing the same phase + roadmap reference.
...
- All new schema files MUST be schema-gated by Alpha 1 PRD-07's snapshot diff.
...
- No multi-axis support (`tabs:`/`axes:`/`<Tabs>`), no library-mode `<Run runtime="…">`, no `<Inspect>` snapshots, no `<Inline>` injection, no `<Skip>` propagation. These are Alpha 3+.
...
- No `DeprecationNotice` runtime propagation. The schema field exists; the runtime path lands at Beta 1.
...
- `core/src/testing/` MUST NOT pull anything from `core/src/cli/oclif/**`. The import-boundary test from Alpha 1 PRD-06 should already catch this.
...
- The schema-snapshot gate (Alpha 1 PRD-07) accepts the new Alpha 2 schemas without manual fixture edits beyond the snapshot diff.
- No story in this PRD increases the public API surface of `@lando/core` outside `/docs/components`, `/testing`, and `/schema`.

## Open Questions

- Final decision on whether shared fixtures live at `docs/guides/fixtures/` or are forbidden in Alpha 2 (forcing each guide to own its fixtures). Default in US-060 is "per-guide overrides shared", revisit if any Alpha 1 guide needs a shared one.
- Whether `runCli` should default to the in-process runner from Alpha 1 PRD-06 unconditionally, or accept a `subprocess: true` option for guides that explicitly want to exercise the compiled binary. Alpha 2 default is in-process only; subprocess form deferred unless a guide needs it.
