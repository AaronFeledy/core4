...
- [PRD-A2-02 US-063](./prd-alpha-2-02-mdx-codegen-pipeline.md) — the generated-file header format this PRD consumes.
...
- [ ] Unknown flags and Alpha-3-only flags (`--variant`, `--step`, `--fixture`, `--update-transcript`) fail with a `NotImplementedError` whose remediation names Phase 3 Alpha 3 and references `spec/ROADMAP.md`.
...
- [ ] The reporter resolves `<guideId>` and `<scenarioId>` from the same headers it used for the rewrite — it MUST NOT regex the file path, because path layout may change in Alpha 3 when variants land.
...
- All flag names MUST match §19.12 exactly to keep author muscle memory portable across the Alpha 2 → Alpha 3 transition even when Alpha 3 unblocks `--variant`/`--step`/`--fixture`.

## Non-Goals

- No `--variant`, `--step`, `--fixture`, or `--update-transcript` flag behavior. They are recognized only to print a Alpha-3-deferred remediation.
...
- No interactive picker if `<guideId>` is omitted — the command prints `lando docs:scenario <guideId> [--scenario <id>]` usage and exits 2. An interactive picker is Alpha 3+.
...
- No watch mode in Alpha 2. `bun run dev:guides` is Alpha 3+ unless a story explicitly pulls it forward.
...
- The reporter MUST tolerate the always-empty `// @variant:` header without misclassifying it as missing. Bug surface is high here because the placeholder will only have content starting in Alpha 3.
