...
| `provider` | Provider selection and adapter initialization | `meta:setup`, `meta:doctor`, `apps:poweroff`, `apps:list --all` |
...
| `DoctorService` | Runs host/app/provider diagnostics and exposes automated or manual remediations; aggregates plugin-contributed `doctorChecks` (§9.5); also records deprecation entries surfaced by `lando doctor --deprecations` (§18.6) | `DoctorServiceLive` (constructed at level `provider` so the selected provider's status can be inspected, and level `provider` already extends `plugins` so plugin-contributed checks register too; transcripts captured via `ShellRunner` per §10.9; self-resilience isolation per §10.9.1) |
