...
| `plugins` | Plugin discovery, manifest validation, contribution graph | `meta:plugin:add`, `meta:plugin:remove`, `meta:doctor` |
| `commands` | Lando command registry services and command-cache refresh ability | command-management and docs/reference commands, `meta:update` (§8.9.7's notify policy requires `commands`-tier or higher, so `meta:update` — a default-family notify command — sits here rather than `plugins`) |
...
- **4.0 status.** The `CommandInvocationCorrelation` schema, the outer-invocation notification-eligibility rule, and the nested-invocation suppression rule above are frozen contract that every current and future invocation path — outer or nested — MUST satisfy. Nested `command:` step invocations specifically are contract-only in 4.0: §8.5.2.1 freezes their shape and this correlation/suppression behavior, but no executor creates a nested invocation at runtime yet. Production nested command-step creation, and the runtime proof that a nested invocation is correlated and suppressed exactly as specified here, is deferred to a dedicated post-4.0 tooling story.
