...
|---|---|---|---|
| `secrets` | value-set + the five secret classes + deep object key-name masking | single `[redacted]` sentinel | `Logger`, `EventService` payloads, `ProcessRunner`/`ShellRunner`/`BunSelfRunner`, `BuildOrchestrator`, `HostProxyService`, `TunnelService`, the CLI failure formatter / bug report |
...
- It is **not** a §4.2 pluggable abstraction. There is no `redactors:` contribution surface. Audited / sandboxed / mirror-aware `ShellRunner`, `BunSelfRunner`, `HostProxyService`, `FileSyncEngine`, `HttpClient`, `Downloader`, and `TunnelService` plugins MUST compose the canonical redactor and MUST NOT weaken it; the relevant contract suites (§13.1) check this.
