...
| `meta:update` | `update` | `commands` | Update Lando core and plugins |
...
**4.0 status: contract-frozen, not implemented.** Everything above is normative *contract* — the `command:` step's shape, resolution rules, error taxonomy, bootstrap-escalation rule, and its `CommandInvocationCorrelation` (§11.2) correlation and notify-suppression behavior (§8.9.7) are frozen SDK-level truth that a future implementation MUST match exactly. No executor ships in 4.0: `ToolingTaskShape` (§8.5.1) does not accept a `command:` step object in its `cmds:` union, there is no compiler pass that resolves `<canonical-id>` or raises `ToolingCommandLookupError`/`ToolingCommandCycleError`, and no runtime path actually assigns a nested invocation its own `invocationId` plus the enclosing `parentInvocationId`. A Landofile author cannot write a working `command:` step against a 4.0 binary today. Building the schema acceptance, the compiler pass, the executor, and the runtime proof that a nested invocation is correlated and notify-suppressed as specified is deferred to a dedicated post-4.0 tooling story — it is explicitly out of Beta 1 scope (see `spec/beta-1/prd-beta-1-00-index.md`'s post-4.0 deferrals table) and is not implied to exist by this section's presence.
...
1. **Registry validation.** Every id in `commands` is checked against the resolved command registry (§8.3 — the same registry `hostProxyAllowed`/allowlist checks use). An id that is not a real canonical command id fails with the existing `ConfigError` at config-resolution time, with `path` naming the offending `notify.commands` entry and `message` naming the unknown id and remediation. This happens before any subscriber registers; no notify-specific error is introduced. **The policy installs at `commands+` bootstrap tier only** (§3.2's bootstrap-level ordering: `commands`, `tooling`, `provider`, `global`, `scratch`, `app` — never `none`, `minimal`, or `plugins`): an id in `commands` that resolves to a command whose bootstrap level is below `commands` fails with the same `ConfigError`, `path` naming the offending entry and `message` naming the ineligible id and commands-tier-or-higher remediation. This is why every id in the default family — including `meta:update`, reclassified to `commands`-tier bootstrap — clears this check; a Landofile-defined tooling task (whose canonical id is always `tooling`-tier or deeper) is a valid, higher-tier target and is not excluded by this rule.
