...
This part defines the CLI surface. **One native command registry and dispatcher** own shipping argv parsing, help, deferred-command plans, and dispatch for both source and compiled modes (§8.4.1). The moment a command's `run()` is invoked, control crosses into Effect and never goes back. OCLIF is not part of the shipping CLI path (superseded dual-dispatch decision; historical spike in §14 Appendix D.1).
...
- `@lando/core/oclif` is not a shipping export (§9). Embedding hosts use `@lando/core/cli` / library APIs.
