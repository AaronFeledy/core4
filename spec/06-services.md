...
**Language dev tooling (formatters and LSP).** The language-runtime service-types (`php`, `node`, `python`, `ruby`, `go`; §6.12.1) MAY contribute, through this same `tooling:` field, the stack's **developer tooling** — a formatter task and a language-server launcher — so a `node` service ships `lando fmt` (prettier/biome) and `lando lsp` (the TypeScript/ESLint language server), a `python` service ships `lando fmt` (ruff/black) and a `pyright`/`ruff` LSP launcher, and so on. This is the provider-neutral, container-first analogue of devenv's `languages.<lang>` bundling of compiler + formatter + LSP: the toolchain lives in the service image, and the tasks are exposed on the host through the normal tooling path (and as bare commands under ambient `shims` mode, §8.2.5). These tasks are pure tooling contributions — **no new schema, no new contract** — and participate in the same conflict precedence and `framework:` selection as any other service-type tooling. A formatter task SHOULD declare `invocation: { mutability: "write", filesystemScope: "app" }` and an LSP launcher `invocation: { mutability: "read" }` (§8.3) so they classify correctly for ambient shims and the MCP surface. Contributing these tasks is the language-tooling story for v4.x; the catalog adds them per language as the framework presets (§6.12.2) mature.
