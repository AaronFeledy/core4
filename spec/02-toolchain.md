...
// 5. Imperative shell (invoked by the native command dispatcher)
export const runStart = async (): Promise<void> => {
  ...
  await Effect.runPromise(program.pipe(Effect.provide(LandoRuntimeLive), Effect.scoped));
};
```
...
- Command-registry metadata generates CLI command reference pages.
...
- `inquirer`, `prompts` — replaced by the `InteractionService` (a Lando-owned prompt service; §3.4), not by `@oclif/core`'s prompt utilities.
- `yargs`, `commander` — the native command dispatcher/registry subsumes these.
...
**Private internal workspace packages are allowed** for ownership seams that would otherwise be enforced only by AST lint (architecture-simplicity US-502/US-516+; core-seams US-535+; scanner-retirement US-551+). The approved seams are `@lando/paths`, `@lando/state-store`, `@lando/landofile`, `@lando/engine`, `@lando/redaction`, `@lando/http-client`, and `@lando/managed-file`; other seams may be promoted when package-dag can carry the constraint and the scanner-retirement ratchet is paid. Public `@lando/core` remains the single publishable runtime + CLI package. Private packages:
