...
- MAY be depended on by `@lando/core`; plugins MAY depend only on private seams declared for plugin use by `check:package-dag` and MUST NOT take runtime dependencies on `@lando/engine` or `@lando/core`.
