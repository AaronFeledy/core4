...
- Delivery reuses the §10.4 relay and provider bridge contract with `kind: "gpg"`: the container sees `/run/lando/gpg-agent/S.gpg-agent` and the feature sets `GNUPGHOME=/run/lando/gnupg` on a tmpfs. Private keys never leave the host.
