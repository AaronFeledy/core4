...
`lando doctor` MUST always emit a structured report on its result channel, and MUST NOT replace that report with a stack trace or an unhandled error, for any host state. (Warnings written to stderr by the CLI framework's own plugin loader, before doctor's program runs, are outside this guarantee.) The only thing that stops a doctor run before it reports is a user interrupt (`Ctrl-C`), which MUST still be honored; a report section that interrupts *itself* MUST be recorded as a section defect instead of ending the run.
...
**Self checks.** A section failure MUST surface as an entry in a new optional `self` report section: `self.checks[]`. Each entry has `name: "doctor-self"`, the failing `section` label, `status: "fail"`, `severity: "error"`, a `reason` of `failure | defect | timeout`, a structured `context` (`section`, `reason`, an optional `failure` tag, and a `message`), and remediation `solutions`. The `message` MUST be redacted (§3.7) and length-bounded before it reaches the report, and the optional `failure` tag MUST be accepted only when it is identifier-shaped. Self checks MUST appear in every output format, including the `ndjson` event stream and its summary counts, and a report carrying any self check MUST exit non-zero so a script or agent cannot read a degraded run as a healthy host.
...
**Plugin check sandboxing.** Each `doctorChecks:` contribution (§9.5) runs under its own deadline with defect capture and plugin/check attribution. A hanging or throwing plugin check MUST become an attributed failed check; it MUST NOT take the doctor run down. A throwing `relevant()` predicate MUST drop only that contribution. Successful payloads MUST be redacted and strictly decoded as `PluginDoctorReport` before report inclusion: at most 32 reports per check, 128-character names/context keys, 2,000-character message/context values, 32 context entries with 16,000 total value characters, and 16 solutions per report. An invalid or over-bound payload MUST be dropped as an attributed plugin-check failure without ending doctor. Deadlines are cooperative: a contribution that blocks the event loop synchronously cannot be interrupted, so this is per-check isolation, not a security sandbox for hostile code.
...
This declared `none` is **command-base bootstrap depth**, not the level-`none` argv fast path in §3.2: doctor resolves through the native dispatcher, runs Effect, and renders through `Renderer`. The command base MUST NOT pre-build a fallible provider runtime before doctor's program starts. Because that guarantee is load-bearing, `meta:doctor` MUST be exempt from §8.9.7 `notify.commands` bootstrap promotion — listing it in `notify.commands` MUST leave effective bootstrap at `none` rather than promoting to `commands` and eagerly reconstructing the runtime doctor isolates. Desktop notifications for doctor are therefore unavailable while safe mode holds; the structured report and non-zero exit on self checks remain the automation contract.
