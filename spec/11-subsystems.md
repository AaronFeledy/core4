...
- The `ssh-agent` feature for `type: lando` defaults to a dedicated sidecar through global `sshAgent.sidecar: true` and per-service `packages.ssh-agent.sidecar:`.
- `sshAgent.sidecar: false` is reserved and rejected in Beta 1. Direct host-agent socket mounts do not ship.
- Alternate feature implementations MAY ship but MUST declare security posture for `lando doctor`.
