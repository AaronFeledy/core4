...
- `bin/lando.ts` short-circuits the level-`none` argv shapes (§3.2) before any heavyweight import resolves; an unknown flag attached to a level-`none` shape falls through to the native command dispatcher (§3.2).
...
- Hot-path runtime caches (`core-command`, `plugin-command`, `app-command`, `cwd-app-map`, `app-plan`) use the §12.2 binary encoding with a versioned magic header; a header mismatch triggers automatic regeneration with no user-visible error. The separately build-embedded command-registry manifest is generated as `core/src/cli/generated/command-registry-manifest.ts` and is not a `CacheService`-managed runtime cache (§12.1, §17.2).
...
- A Bun program can resolve an `App` handle through `openLandoRuntime(...).app(...)` or Layer-native `resolveApp(...)`, then start, info, exec, and stop that app programmatically through `app.start()`, `app.info()`, `app.exec()`, and `app.stop()` while receiving tagged errors with remediation messages.
- A long-lived embedding host (TUI, editor extension, dashboard, web server) can construct one retained `LandoRuntime` object, execute many sequential `runtime.app(...)`, `app.info()`, `runtime.run(runTooling(...))`, and `runtime.run(appConfig.get(...))`-style operations against it, and have operations 2..N each meet their respective §2.1 hot-path budget at p95 without paying repeat bootstrap cost. The library-mode reuse-perf test class in §13.1 asserts this on per-PR CI.
...
- The default `RouterService` Live Layer (`RouterServiceTraefikGlobalAppLive` in `@lando/proxy-traefik`) realizes its routes through a `traefik` service running inside the global app; the plugin contributes both `routerServices:` and `globalServices:` entries, and installing one without the other fails at plugin load with `ProxyContributionPairError` (§20.10.1).
...
- **Command framework** — The pluggable `CommandFramework` abstraction (§4.2) that handles argv parsing, manifest, help, namespace-to-topic mapping, and top-level alias registration. The default implementation is the native command registry and dispatcher shared by source and compiled entries (§8.4.1); OCLIF was the historical default (§14 Appendix D).
- **Command namespace** — One of the three core namespaces (`app`, `apps`, `meta`) or a plugin-owned topic. Each namespace is a top-level native registry topic (§8.4) and the prefix segment of every canonical id within it.
