...
| Plugin postinstall trust model and `lando plugin trust*` command surface | `meta:plugin:trust <name>` (referenced in §9.6) and `meta:plugin:trust-authoring-root <abs>` (referenced in §9.10.4) populate the trusted-postinstall allowlist at `<userConfRoot>/plugin-trust.yml`. Open decisions: the exact command flags (single global allowlist vs. per-source policies), whether trust grants expire, whether GA ships an explicit `lando plugin trust list` / `revoke` pair, and whether trust is required only for npm/registry sources or also for git/local. The architecture preserves the option (`PrivilegeService` is the elevation primitive, `<userConfRoot>` is the config root); the user-visible surface is the GA-blocking decision. |
| Compiled-binary CLI dispatch unification | The compiled `$bunfs` binary currently bypasses OCLIF at runtime and routes through `runCompiledCli` in `core/src/cli/run.ts` (§8.4.1). Options: (a) make `@oclif/core`'s `execute()` dispatch reliably inside `bun build --compile` against the precomputed `oclif.manifest.json` + `compiled-commands.ts`, then delete `runCompiledCli` and the §8.4.1 interim relaxations; (b) accept dual dispatch as permanent, promote §8.4.1's parity rules to normative, and add a compiled-binary parity test layer to §13.1. Spike scheduled in ROADMAP Phase 3 Beta; decision GA-blocking. **Spike complete (see §14 Appendix D.1): option (a) is unreachable through any supported `@oclif/core` public API (`Config.load` → `findRoot` and the `module-loader` runtime `import()` both break inside `$bunfs`); recommended outcome is option (b), with the two shipping paths already proven at parity. Application of the outcome (promote §8.4.1, ship the parity test layer) remains open.** |
