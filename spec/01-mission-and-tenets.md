...
5. **Hard reset.** v3 services, v3 service inheritance, v3 recipes, legacy raw passthrough shims, and Traefik labels are not part of core. Core MUST NOT execute any Lando 3 code path. Explicit conversion of Lando 3 configuration is provided by the bundled `@lando/lando3` plugin through the two-way config-translation surface (§7.4.1/§9.5). Conversion MUST run only on explicit request, never during bootstrap or `start`.
...
| **Native CLI dispatcher for UX, Effect for logic** | One native command registry and dispatcher own argv parsing, command discovery, help, and shipping binary entry for both source and compiled modes (§8.4.1). The moment a command's `run()` is invoked, control crosses into Effect and never goes back. OCLIF is not part of the shipping CLI path (historical spike: §14 Appendix D.1). |
...
| **Agent-native dev environment** | AI coding agents are a first-class operator, not just humans and scripts. Two defaults are reviewable. **Context continuity:** when work crosses a boundary (host into container, command into command), the context an agent depends on is carried across rather than dropped — governed by explicit, security-fenced policy so continuity never becomes a credential-exfiltration path. **Machine-legibility:** every command exposes structured, predictable output an agent can consume without scraping prose, and failures are tagged errors carrying machine-readable cause and remediation. When a choice would force an agent to parse human output or lose state at a boundary, the legible, state-preserving approach is preferred. Adding a user-facing surface an agent cannot drive without screen-scraping is a defect, not a follow-up. |
...
| Exact Compose compatibility subset | **Resolved: schema-backed top-level subset for Beta 1.** Landofiles accept the Compose vocabulary classified by the committed disposition matrix and published at `docs/reference/compose-key-matrix.mdx` when values match Lando's supported schema subset. Compose `version:` is accepted for compatibility and deprecated; unsupported top-level Compose keys fail closed with remediation pointing to `includes:`, `providers.<provider-id>`, or a config translator. |
| OCLIF major version | **Superseded by architecture-simplicity (US-500+):** OCLIF is removed from the shipping CLI path. The historical pin to OCLIF v4 applied only while dual dispatch was in force; it is not a shipping dependency after the single native dispatcher lands (§8.4.1). |
...
| Compiled-binary CLI dispatch unification | **Superseded (architecture-simplicity):** option (b) dual dispatch is **not** permanent. The spike (§14 Appendix D.1) remains valid historical evidence that `@oclif/core`'s `execute()` cannot dispatch inside `bun build --compile` through any supported public API — and that evidence **motivates removing OCLIF from the shipping path**, not keeping two engines. **Resolution:** one native command registry + one dispatcher for source and compiled modes (§8.4.1); drop dual-path parity tests; keep registry completeness, machine-output conformance, and relocated-binary smoke (§13.1). Tracked in `spec/architecture-simplicity/` (US-522..US-531). |
...
| **Service-type `topLevelAlias:` enforcement** | §10 already says service-type tooling MUST NOT use `topLevelAlias:`. The key is in `BETA_TOOLING_TASK_KEYS` and is rejected for every Landofile tooling task today, so the service-type-only rule is vacuous. Enforce it when the 4.1 tooling schema graduation lifts that beta gate. |
