...
- [`spec/alpha/prd-alpha-07-ci-distribution-and-release-channel.md`](../alpha/prd-alpha-07-ci-distribution-and-release-channel.md) — the CI baseline this PRD extends.
...
- Ship `scripts/build-recipe-readmes.ts` strip/flatten **only if** an Alpha recipe README under `recipes/<id>/README.mdx` contains executable components by the time this PRD lands. Otherwise the strip/flatten generator is deferred to Beta.
...
**Status:** **Conditionally in scope.** Required only if any Alpha recipe (PRD-04 deliverables) ships its README as MDX containing executable components by the time this PRD lands. If no Alpha recipe README is MDX, this story is closed as "Not applicable in Alpha 2; deferred to Beta per §19.13" and the strip/flatten generator is not implemented.
...
- [ ] For each `recipes/<id>/README.mdx` that contains a `<Guide>`, the script writes `recipes/<id>/.scaffold/README.md` (Alpha 2 emits a single prose-only output; the per-axis-value cell-product output from §19.13 is Beta+ because Alpha 2 has no axes).
...
- [ ] The recipe scaffold path (PRD-04 US-031 in Alpha) reads `.scaffold/README.md` if present, falling back to `README.md` otherwise.
...
**Decision-tree:** This story's status is decided in the PR series that opens Alpha 2. If at PR-open time no `recipes/*/README.mdx` exists with executable components, the maintainer marks the story closed in the PR description and updates `progress.txt` (mirroring `spec/alpha/progress.txt`) to reflect deferral.
...
- The lint rules implemented in US-072 are an explicit allow-list; adding any new rule is a Beta-or-later change that requires a new PRD story.
...
- The CI workflow file `.github/workflows/ci.yml` is generated from `scripts/build-ci-workflow.ts` — hand edits are forbidden. The drift gate from Alpha PRD-07 still applies.
...
- Reuse the Alpha PRD-05 redaction module rather than introducing a parallel one. If divergence is needed, surface it as a separate PR with an explicit decision rather than copy-pasting.
...
- The decision threshold for promoting a recipe README from `.md` to `.mdx` is owned by Alpha PRD-04. Alpha 2 only reacts to that choice — it does not push for one.
