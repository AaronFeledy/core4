...
- `kind: tooling` resolves at app-plan compile time and shares the `namespace`, `flatten`, `internal`, `optional`, `aliases`, `excludes`, and `vars` contract defined for `toolingIncludes:` in §8.5.8. The `toolingIncludes:` shorthand key remains available and is not deprecated: it and `includes: [{ kind: tooling, ... }]` normalize through the same resolver and produce equivalent plans, with no compatibility shim or dual resolution path.
