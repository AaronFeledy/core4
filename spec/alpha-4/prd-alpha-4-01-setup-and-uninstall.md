...
- [ ] `lando setup` calls the same `ensureRuntime` as its final readiness step (setup does not own the socket) and preflights before detached launch. Rootless prerequisites are classified with actionable, per-prerequisite remediation (missing `/etc/subuid`+`/etc/subgid`, missing `newuidmap`/`newgidmap`, cgroups v2 delegation, missing `XDG_RUNTIME_DIR`), distinct from a generic launch failure. On exactly Ubuntu 26.04, explicit setup may consentingly install the fixed `uidmap` package through `PrivilegeService`, re-probe, and continue; every other host/prerequisite remains fail-closed. Provision/preflight, launch, and readiness are distinct terminal task-tree steps.
...
- FR-2: `lando setup` MUST accept `--yes`, `--no-interactive`, `--provider=<id>`, `--skip-provider`, `--skip-proxy`, `--skip-install-ca`, `--skip-shell-integration`, `--skip-file-sync`, `--runtime-bundle-url`, and `--runtime-bundle-sha256`.
...
- FR-20: `ensureRuntime`'s launch MUST use an injectable seam, MUST be idempotent (a reachable socket is not relaunched; a stale PID/socket is reaped and relaunched), and MUST preflight rootless prerequisites (`subuid`/`subgid`, `newuidmap`/`newgidmap`, cgroups v2 delegation, `XDG_RUNTIME_DIR`) before detached launch with per-prerequisite remediation distinct from a generic launch failure. Explicit `lando setup` MAY provision missing uidmap helpers only for exact Ubuntu 26.04 after consent (`--yes` grants, interactive prompts once, `--no-interactive` alone denies), using fixed `apt-get` argv through `PrivilegeService` and re-probing before launch; unrelated commands and unsupported hosts MUST NOT auto-provision or fall back to an external socket. Setup MUST expose provision/preflight, launch, and readiness as explicit task-tree steps and settle every child on failure.
...
- On hosts where rootless `podman system service` cannot run, `lando setup` fails with per-prerequisite remediation and never falls back to an external/system socket. The sole package-provisioning acceptance path is consented `uidmap` installation on exact Ubuntu 26.04 during explicit setup.
