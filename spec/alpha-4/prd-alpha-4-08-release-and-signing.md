...
- Who owns release credentials (Apple notarization, Windows certificate, cosign/OIDC identity) for CI, and are they available before RC? Resolved by decision: unassigned for Beta 1, so the full signed release runs **manually until RC** and is not wired into CI. The generated `release` workflow stays scoped to dev prereleases only; maintainers cut `4.0.0-beta.N` by invoking `scripts/release.ts` per `docs/release-runbook.md`. Full-pipeline CI (with credential-gated signing stages that skip cleanly when secrets are absent) is an RC gate that reopens once credential owners are assigned. See `docs/beta-1-decisions.md` for the full record.
