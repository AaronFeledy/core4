...
- [ ] With `LOCAL_REHEARSAL=1`, stages 9, 10, 11 signature work, 12, and 13 skip with warnings when credentials are absent.
...
**Acceptance Criteria:**
- [ ] Linux release artifacts produce a `SHA256SUMS` manifest covering linux-x64 and linux-arm64 binaries.
- [ ] `SHA256SUMS` is GPG-signed in the Sign stage when release credentials are present.
- [ ] `SHA256SUMS` is cosign-signed in the Sign stage.
- [ ] Verification confirms checksum coverage and both signatures before Manifest generation.
