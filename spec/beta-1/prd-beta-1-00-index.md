...
| Tooling `command:` step (§8.5.2.1) production implementation: `ToolingTaskShape` step-object schema, the compiler pass (`ToolingCommandLookupError`/`ToolingCommandCycleError`), the executor, and runtime proof that a nested invocation carries its own `invocationId` plus the enclosing `parentInvocationId` | US-458/US-459 freeze only the `CommandInvocationCorrelation` schema and the notify-suppression contract for a future nested invocation; no executor exists in 4.0 and `ToolingTaskShape` rejects step objects today | dedicated post-4.0 tooling story (not a numbered Beta 1 PRD) |
