...
> **Top-priority insertion (PRD-19 — service composition).** [PRD-19](./prd-beta-1-19-service-composition.md) (US-355..US-362) is a remediation PRD that corrects a structural omission discovered mid-Beta-1: the v4 service model (§6.1, §6.11) — a service is a `base` (`l337`/`lando`) plus priority-ordered `features` — was never implemented; service types hand-build `ServicePlan`s instead. Its stories carry the **lowest priority indices (1..8) in `prd.json`**, ahead of all other in-flight Beta 1 work, and it blocks any later story that adds a service type or feature. Land it first.

> **Top-priority remediation (PRD-01 — runtime-socket autosetup).** [PRD-01](./prd-beta-1-01-setup-and-uninstall.md) US-363..US-367 (added post-Alpha-3) close a second structural omission found in Beta 1: `lando setup` verifies the runtime bundle but never extracts it or launches the Lando-owned Podman API socket, so the default `@lando/provider-lando` only worked when handed an external socket via the `LANDO_TEST_PODMAN_SOCKET` test variable. These stories make `lando setup` stand up a working runtime end-to-end (extract binaries, launch + own the private socket, idempotent re-detection, Paths-driven wiring) and switch CI to the Lando-managed socket with no manual `podman system service`. They carry **top-band priorities (1..5) in `prd.json`**, alongside the other top-priority insertions.
...
| -- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------- |
| 01 | [Setup & uninstall completion](./prd-beta-1-01-setup-and-uninstall.md)                  | full `lando setup` across platforms, idempotency/re-entrancy, readiness summary, first-class `lando uninstall`, dual-path parity, runtime-socket autosetup | US-200..US-207, US-363..US-367  | —                       |
...
| 09 | [Supply chain, self-update & verified downloads](./prd-beta-1-09-supply-chain-and-self-update.md) | SBOM/SLSA/cosign release artifacts, signed update manifest, atomic re-exec/rollback, the `HttpClient`/`Downloader` network primitives + tool provisioning, and the frozen `TunnelService` contract (public-sharing surface; feature is 4.1) | US-258..US-265, US-285..US-288, US-330..US-332, US-342..US-343 | PRD-01, PRD-04, PRD-08, PRD-11, PRD-13, PRD-14 |
