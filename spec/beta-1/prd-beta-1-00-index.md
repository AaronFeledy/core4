...
This PRD set picks up at **US-372** (Alpha 4 ended at US-371) and runs through **US-424**: US-372..US-395 are the remediation stories, US-396..US-409 are the feature-wave stories, US-410..US-412 (PRD-10) close the runtime-bundle distribution gap — the committed `runtime-bundle-versions.json` is a placeholder pointing at a repository that will never exist, so no binary can complete `lando setup` without the `LANDO_RUNTIME_BUNDLE_MANIFEST` override — and US-413..US-424 (PRD-11) lock the Podman 6 runtime contract that PRD-10's publishing work must ship. PRD-10 stands up the in-repo `runtime-v*` publishing pipeline and the §5.8.1 committed-manifest invariant so local builds, dev snapshots, and releases all resolve the runtime bundle with zero overrides.
