...
- [ ] A new generated workflow (emitted by a `scripts/build-runtime-bundle-workflow.ts`-style generator following the existing `build-release-workflow.ts` pattern; never hand-edited YAML) builds runtime bundles for every supported host platform, keyed by runtime host key: `linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64` (the §13.5 release platform id `windows-x64` corresponds to manifest host key `win32-x64`; both names stay in their existing domains per root `AGENTS.md`).
