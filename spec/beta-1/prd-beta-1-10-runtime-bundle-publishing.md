...
- [ ] A new generated workflow (emitted by a `scripts/build-runtime-bundle-workflow.ts`-style generator following the existing `build-release-workflow.ts` pattern; never hand-edited YAML) builds runtime bundles for every §13.5 host target: `linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`.
- [ ] Bundle contents are assembled from **pinned** upstream artifact versions declared in a committed input (not "whatever the runner's package manager installed"): Podman + rootless helpers (crun/conmon/netavark/aardvark-dns/gvproxy/fuse-overlayfs/slirp4netns/newuidmap/newgidmap as applicable per platform) on Linux; the Podman remote client + machine/gvproxy helpers on macOS and Windows. Assembly is reproducible: re-running against the same pins produces artifacts with identical SHA-256s.
...
- [ ] `plugins/provider-lando/runtime-bundle-versions.json` contains real HTTPS URLs under this repository's `releases/download/runtime-v<version>/` path, real SHA-256 checksums, and real sizes for all five host targets; the `_comment` placeholder disclaimer is gone.
...
- [ ] The periodic/release-path verification asserts each manifest URL resolves over HTTPS to a 200 with the recorded `Content-Length` (per the §17.2 catalog row), without downloading full artifacts on every PR.
