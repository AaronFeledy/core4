...
| End-to-end | `bun test` + `Bun.# Lando v4 — Testing, Distribution, and Quality Gates
...
| Deprecation | `bun test` + `@lando/core/testing` | Per-surface tests that exercise every `DeprecationNotice` in the codebase: triggering the surface emits the `deprecation-used` event, the renderer warns once per `(kind, id)`, `lando doctor --deprecations` lists the entry, and the `removeIn` gate rejects stale notices (§18.7–§18.8) |
| Perf budget | `bun test` + `Bun.$` driving `dist/lando-${target}` with byte-resolution stdout/stderr capture | Asserts the §2.1 end-to-end and perceived-performance budgets, the §12.5 hot-path read budgets, and the §8.9.1 first-paint contract |
 against the compiled binary | The released artifact run on a real OS against a real provider |
...
**Post-GA contract suites (devenv-parity work).** The primitives specced in §10.11–§10.15 ship with their features per the Phase 8–10 sequencing (ROADMAP), and each extends an existing test layer rather than adding a new one: the **Effect service** layer gains `CheckRunner` (§10.11) tests (DAG ordering, `skipIf` up-to-date short-circuit, `failFast`/`continue` policy, deterministic `CheckResult`) and `ProcessSupervisor` (§10.13) tests (readiness-gated `dependsOn` ordering, restart policy, `Scope` finalization stopping every supervised process within the §10.10-class interrupt budget, stdout redaction); the **schema gate** (§13.2) gains round-trip coverage for the new frozen schemas `CheckResult` / `Diagnostic` (§10.11) and `InvocationPolicy` (§8.3); the **host proxy contract** discipline extends to an MCP-exposure contract (§10.14) asserting only `mutability: "read"` commands are agent-exposed by default and that write/lifecycle/host-mutation exposure requires `requiresConfirmation`; and a **secret-store backend** contract (in `@lando/sdk/test`) asserts each `SecretStore` backend (§4.2) honors selection precedence and never leaks decrypted values into caches/logs/telemetry. The §13.4 governance gate additionally fails any command that declares an `invocation.exposure` without a `mutability`.
