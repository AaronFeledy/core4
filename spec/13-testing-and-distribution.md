...
| Service composition contract | Shared contract suite (in `@lando/sdk/test`) | Any `ServiceType` (canonical or plugin) must pass `runServiceCompositionContract` (§6.11.0.1): it declares a `base` (`l337`/`lando`); `resolve()` returns a `ServiceTypeResolution` and never hand-builds a `ServicePlan`; core composes `base + features` through the §6.11.0 pipeline; a `lando`-base service carries the `lando.env` env layer + the §6.9 `LANDO_*` identity env; an `l337`-base service carries **no** injected `LANDO_*` env layer and no `/etc/lando` scaffolding; the env-layer helper is reachable only via the `lando.env` feature (the §13.4 boundary gate forbids direct imports); `extends:` depth ≤ 4 with no cycles; feature priority order and resolution are stable across replays; the resolved `base` + ordered `FeatureRef` list participate in the app-plan cache key |
| Service feature contract | Shared contract suite (in `@lando/sdk/test`) | Any `ServiceFeature` (and every built-in `lando.*` feature) must pass `runServiceFeatureContract`: `apply` mutates only the in-memory draft context, is deterministic and idempotent across replans/rebuilds, emits provider-neutral plan intent only (no capability checks / bind realization / host-port decisions inside a feature), runs at its declared `priority`, gates provider-specific behavior on declared `requires:` capabilities, and surfaces conflicts as `ServiceFeatureError` rather than silent overwrite |
| App feature contract | Shared contract suite (in `@lando/sdk/test`) | Any `AppFeature` (§6.11.4; e.g. the mailpit SMTP-injection feature) must pass `runAppFeatureContract`: `activatedBy` gates invocation (no match → no `apply`, no plan-cache entry); selectors resolve against the **resolved service drafts** (stages 1–3 of §6.11.0), never the not-yet-emitted finalized `AppPlan`; the app-feature phase runs after the entire service-feature phase; mutators apply idempotently to each selected service; a cyclic A↔B mutation is rejected with `AppFeatureCycleError`; `requires.globalServices` aggregates into the user app's `pre-start` ensure-running pass and a missing global service fails with `GlobalServiceMissingError` + remediation; `AppFeatureError` is the documented tagged union with the contributing plugin id |
...
| TunnelService contract | Shared contract suite (in `@lando/sdk/test`) | Any `TunnelService` plugin must pass: capability declaration matches observed behavior; target resolution accepts only app `RoutePlan` / service endpoint targets and refuses arbitrary host ports unless explicitly allowed; control-plane egress is issued through the resolved `HttpClient`; connector binaries are provisioned through the tool-provisioning helper / `Downloader` rather than plugin-local downloads; foreground sessions are `Scope`-bound and finalize connector processes on `Effect.interrupt`; detached sessions are recorded in a `StateStore` bucket and stale PID/socket entries reconcile safely; `pre-/post-tunnel-start`, `tunnel-ready`, `pre-/post-tunnel-stop`, and `tunnel-status` events redact provider tokens, auth URLs, public hostnames when configured, and local paths; readiness uses the §10.5.1 probe primitive instead of ad-hoc retry loops; the implementation never routes local/volume byte movement through the tunnel service |
...
| RemoteSource contract | Shared contract suite (in `@lando/sdk/test`) | Any `RemoteSource` plugin (and the built-in `local`/`rsync`/`ssh`/`url`/`s3` remotes) must pass (§10.12): capability declaration matches observed behavior; `listEnvironments`/`resolve` resolve `<remote>[@<env>]` deterministically and fail `RemoteEnvNotFoundError`/`RemoteDatasetUnsupportedError` for unknown env/dataset; all control-plane + byte egress is issued through the resolved `HttpClient` (no direct `fetch`/socket); vendor CLIs are provisioned through the tool-provisioning helper / `Downloader`; `fetch`/`send` are `Scope`-bound and finalize on `Effect.interrupt`; the local landing half is delegated to a `Dataset` + `DataMover` and never re-implemented; `push` is rejected when `capabilities.push` is false and a `protectedByDefault` env requires `--force`; tokens/auth URLs/host paths are redacted in `pre-/post-pull`/`-push`/`-dataset-fetch`/`-dataset-send` events; readiness uses the §10.5.1 probe primitive |
| Dataset contract | Shared contract suite (in `@lando/sdk/test`) | Any `Dataset` plugin (and the built-in `files` dataset; the `database` dataset ships in `@lando/sql`) must pass (§10.12): `capture` produces and `apply` consumes a portable `DataEndpoint` exclusively through `DataMover` (no hand-rolled byte movement); `capture`→`apply` round-trips a dataset's bytes; `apply` is destructive-by-contract and reports `localStore` so the orchestrator can auto-snapshot first; DB credentials ride env, never argv; a binding that would write into the app code tree fails `DatasetBindingError`; `capture`/`apply` are idempotent/replay-safe and emit redacted `pre-/post-dataset-capture`/`-apply` events |
| Managed-file contract | Shared contract suite (in `@lando/sdk/test`) | The core `ManagedFileService` and host/test overrides must pass (§10.13): create/update/skip-unchanged/skip-adopted/conflict/adopt/release/remove; `plan` matches `apply`; atomic replace leaves no torn file under `Effect.interrupt`; realpath escapes fail; markers round-trip per format; `block` mode is idempotent; the ledger is a `StateStore` bucket with quarantine/locking semantics; known secrets never appear in `ManagedFile` events/history/transcripts. This is StateStore-style integrity-invariant coverage, not the §4.2 plugin-abstraction kit |
| Plugin SDK contract | Type tests + runtime tests | Public API compatibility |
| Machine-output conformance | `bun test` + `@lando/core/testing` driving every canonical command id | Asserts the §8.11 universal `--format json` guarantee: every non-interactive canonical command (and the deferred-command stubs) accepts `--format json` / `--json` / `-j` and emits a `CommandResultEnvelope` that decodes against the published schema, with correct `command` and `ok` for a success case and a failure case; every `LandoCommandSpec` declares a `resultSchema` (registration fails otherwise); streaming commands emit `StreamFrame`s terminated by a `result` frame; the OCLIF and compiled `runCompiledCli` paths produce identical envelopes (cross-checked by the §13.1 dispatch-parity layer); secret values are absent from every envelope (redacted via §3.7) |
...
**Plugin-abstraction contract-kit layer coverage.** Each §4.2 plugin-abstraction that publishes a shared contract suite from `@lando/sdk/test` (`make*ContractSuite` / `run*ContractSuite`) MUST be exercised against its built-in implementation(s) by a `core/test/**` invocation, never an `sdk/test/**` self-test. A single layer-coverage gate (`core/test/contract/plugin-abstraction-coverage.test.ts`) holds a canonical manifest mapping each of the six kit abstractions (`ToolingEngine`, `RouteFilter`, `SecretStore`, `ConfigTranslator`, `PluginSource`, `DoctorCheck`) to its suite exports and its built-in-invocation file(s), and FAILS when a published suite has no built-in invocation, when an invocation file is deleted or stops calling its suite, or when a kit suite export goes missing/renamed. An abstraction whose §4.2 row declares "None bundled by default" (e.g. `ConfigTranslator`) is marked `none-bundled` in the manifest — a principled exception that still requires the suite exports but no core built-in, since no built-in ships to run. Abstractions that are schema-only in core today (`RouteFilter`) run the suite over the spec's documented reference transforms (a `reference-mirror` invocation), and `DoctorCheck` adapts the real `doctor()` selected-provider check rather than a fabricated check object. The suites' own negative cases (a non-idempotent filter, a leaking transcript, a non-deterministic translator) prove a plugin implementation cannot satisfy the interface while weakening containment / redaction / determinism guarantees.
...
- The machine-output boundary gate passes: `bun run check:machine-output` (§8.11) walks `core/src/**` and `plugins/**` and fails on any `JSON.stringify` of a command result outside the single `encodeCommandResult` seam, and on any `LandoCommandSpec` registered without a `resultSchema`; per-command `render*` helpers may produce `text`/`table`/`yaml` only.
