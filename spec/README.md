...
| 02 | [`02-toolchain.md`](./02-toolchain.md) | §2 | Bun policies, single-binary build, performance budgets. TypeScript strict settings. The landed CLI framework (one native command registry and dispatcher shared by source and compiled entries) plus the historical OCLIF-adoption rationale. Effect runtime model and reference patterns. Effect Schema as the single contract language, including schema annotations, JSON Schema output, and generated reference docs. The Starlight docs toolchain. Forbidden runtime dependencies. The `@lando/core` package surface and entry-point catalog. |
| 03 | [`03-architecture.md`](./03-architecture.md) | §3 + §11 | The four concentric layers (imperative shell → Effect runtime → ports → adapters), bootstrap flow with declared `BootstrapLevel`s, source layout, the catalog of core Effect services, lifecycle event scopes, and the imperative-shell catalog (CLI plus embedding hosts). Plus the `EventService` deep dive: payload schemas, subscriber priority bands, bounded subscriber selectors (exact event or the precomputed `cli-command-terminal` family) and lazy/cached `SubscriberFactory` invocation (§11.3.1), the standard cold-start sequence, the hot-path tooling subset, and subscriber failure handling. |
| 04 | [`04-pluggability.md`](./04-pluggability.md) | §4 | The master pluggability catalog: every replaceable abstraction with its Effect Service tag, default implementation, and swap mechanism, including two-way config translators (decode plus optional encode, bundled `lando4`, `recipe`, and decode-only `lando3`) invoked only on explicit request. Selection precedence rules, manifest contribution shape, and mandatory abstraction guarantees. |
...
| 06 | [`06-services.md`](./06-services.md) | §6 | The v4 service model: `l337` and `lando` bases, the common service schema with supported Compose service keys, the group-weighted artifact build, app mounts and mounts (with excludes/includes semantics), storage scopes, auto-naming, and default planned-user `home:` storage, endpoints/hostnames/routes with shorthand normalization and provider-neutral filters, per-step build users, healthchecks, certs and additional CA injection, the `LANDO_*` environment contract, the `ServiceInfo` schema, the `ServiceType` + `ServiceFeature` contracts (with the built-in feature priority list), the canonical service-type catalog shipped in core (PHP, Node, Python, Ruby, Go, common databases, caches, search, mail, queues, static, raw Compose passthrough) with its single shipped version matrix and file-backed catalog options, and the build orchestration model (parallel per-service artifact and app-build phases via `BuildOrchestrator`, the `BuildPlan` DAG with per-phase concurrency caps and failure policies, the content-hashed `buildKey` up-to-date check, and per-step transcript artifacts). |
| 07 | [`07-landofile-and-config.md`](./07-landofile-and-config.md) | §7 | Landofile discovery and bounds, the six-file merge order with array identity keys, the `load()` / `import()` file-IO expression helpers (with `FileRef` / `ImportRef<T>` value shapes and pipe-decoder pipeline), the config-wide expression language (`{{ … }}` interpolation, filter-and-call helpers, native `${VAR}` shell-parameter-expansion, AST + staged bootstrap-level-aware resolution), the pluggable `TemplateEngine` abstraction with the bundled `lando` / `handlebars` / `mustache` engines, the top-level Landofile keys, the supported Compose subset, explicit set-based config translation through the expression-aware authoring IR (`LandofileAuthoringShape` / `LandofileAuthoringFragment`, tagged `landofile-document-set` / `recipe-request` inputs, the six diagnostic kinds, the encode round-trip law), the loader failure path (`Lando3LandofileDetected`, `LandofileDialectMixError`), the inert `recipe:` declaration with mandatory producer provenance and the `recipe.<option>` expression scope, the explicitly forbidden wrapper keys (`compose:`, `recipes:`), bounded global `appEnv` / `appLabels` maps, `router.enabled` and the `scanner:` union, the global config schema, env-var override naming, the `includes:` composition primitive with its source schemes (local/git/npm/registry) and lockfile, and schema / generated-doc publication. |
| 08 | [`08-cli-and-tooling.md`](./08-cli-and-tooling.md) | §8 | Command kinds (built-in, plugin, tooling, management). Three first-class command namespaces (`app`, `apps`, `meta`) with plugin-owned topics. Top-level alias mechanism and conflict rules. Built-in command list with behavioral requirements, including dedicated `app config` (`translate --to`, `explain`, `migrate`), `meta config`, and `meta recipes:*` commands, service-scoped `app:rebuild` / `app:info --service`, and the `pre-restart` / `post-restart` bracket. The `LandoCommandSpec` contract (with `namespace` and `topLevelAlias` fields) and `CommandInput` shape. Single native dispatch policies (registry-first manifest, hooks bridging to Effect, `SIGINT` → `Effect.interrupt`, bounded flexible taxonomy, namespace-to-topic mapping), with the retired OCLIF design preserved as historical context. Taskfile-inspired tooling schema with normalized flag/arg metadata, ordered steps, dynamic service resolution, expressions, dependencies, includes, up-to-date checks, and event names extended from resolved tooling. The `ToolingEngine` abstraction and its hot-path cache. `lando apps init` and the v4 recipe model — Yeoman-style init-time scaffolds with Q&A prompts (text/select/multiselect/confirm/number/secret/path/editor), file manifests, and post-init actions, replacing the v3 recipe-as-plugin model; recipes decompose into authoring fragments with inert producer provenance, declarative `migrations:` snapshots, and secret dispositions (`secret-store` reference or a named init-only sink). The `Renderer` service and selection precedence, the first-paint contract, and the concurrent task tree contract that drives the multi-task build UI (per-task tail panels, alt-screen full-tail expand). The default-renderer implementation contract (OpenTUI substrate, split-footer live region, degradation, frame-snapshot testing), the rich render events (`code.snippet`/`diff.render`/`markdown.block`), plugin-contributed renderer panel slots, the keymap action vocabulary, desktop notifications (`notify.desktop` + `@lando/notify-lando`), and the interactive `app:logs --follow` viewer. The `InteractionService` — the input peer of `Renderer` — that owns the published prompt vocabulary (`PromptSpec`), answer-source precedence, interactivity-mode resolution, and `secret` redaction for every prompting surface (recipes, plugin authoring, setup, doctor), and is pluggable for headless/CI, recording/test, and GUI/host transports. |
...
| 10 | [`10-plugins.md`](./10-plugins.md) | §9 | Plugin identity (manifest locations, distribution forms via `PluginSource` adapters), runtime rules, discovery order across bundled/system/user/app-local sources (CLI default; library mode in §16.4), the full manifest schema (with command-namespace and top-level-alias rules) including the frozen bounded-selector `subscribers:` shape and `rendererPanels:`, the contribution surfaces table, the install/update flow for `meta:plugin:add` / `meta:plugin:remove` / `meta:update` (and their top-level aliases), plugin loading rules, and the constrained `LandoPluginContext` (including the `events.publishRender` render-event seam). |
| 11 | [`11-subsystems.md`](./11-subsystems.md) | §10 | Networking intent, `RouterService` and `RoutePlan` (with the route-filter abstraction), `CertificateAuthority`, corporate proxy/custom CA handling for Lando-owned network access, the `HttpClient` outbound-egress chokepoint and the `Downloader` verified artifact-acquisition primitive (plus the tool-provisioning helper that installs pinned host binaries over it), SSH and host identity (with the new sidecar-by-default SSH-agent design), `HealthcheckRunner` and `UrlScanner`, files and performance (with the `FileSyncEngine` pluggable abstraction and the bundled `@lando/file-sync-mutagen` reference engine that transparently accelerates bind mounts on Docker-Desktop-class providers), SQL helpers (plugin-only), `lando setup` and host integration, logs/diagnostics, and the `DataMover` local/volume byte-movement chokepoint (§10.11) with its snapshot/restore store. |
| 12 | [`12-caches-and-persistence.md`](./12-caches-and-persistence.md) | §12 | The cache catalog (command, plugin, app-plugin, app-plan, service-info, provider, update) plus the build-embedded command-registry manifest used by router bootstrap. Encoding choices. Atomicity rules. The full list of persistent on-disk artifacts, including the managed-file transaction journal, stage, and backup lifecycle. Hot-path read budgets. Disconnectable local-dev state after app build. |
| 13 | [`13-testing-and-distribution.md`](./13-testing-and-distribution.md) | §13 | The test layers (unit, Effect service, CLI, library API, provider contract, template engine contract, host proxy contract, plugin SDK contract, scenario, recipe, executable guides and generated scenarios, deprecation, perf budget, end-to-end, and the renderer-substrate layers), Effect testing patterns, the mandatory provider contract suite, the library API contract suite, the recipe suite that exercises every canonical recipe, the scenario / recipe / end-to-end conventions that replace the Lando 3 Leia format, schema and docs gates, type gates, PR merge requirements, the two distribution forms (compiled binary and library package whose `package.json#bin` doubles as a CLI install path) plus bundled-plugin and bundled-recipe generation, the docs site build, the per-PR/nightly/weekly CI matrices, and the release flow with channels. |
| 14 | [`14-appendices.md`](./14-appendices.md) | §15 | Provider-neutral language reference. Forbidden core dependencies. The source-derived acceptance checklist (with embedding criteria). The historical OCLIF-vs-`@effect/cli` decision rationale, superseded by the native single-dispatcher target. The glossary. |
| 15 | [`15-binary-build-and-release.md`](./15-binary-build-and-release.md) | §17 | The build pipeline and its single orchestrator. The codegen catalog (every generator, its inputs, outputs, and staleness gate). Asset embedding policy (hybrid: static JSON imports for small data, `Bun.embeddedFiles` for large data), plus `scripts/build-compiled-binary.ts`'s `onResolve` build plugin that prunes 7 of the 8 catalog OpenTUI native packages to generated stubs and embeds exactly the one matching each compiled release target (§17.3.1). Per-platform signing and notarization (macOS Developer ID + notarytool, Windows Authenticode + cosign, Linux GPG-signed checksum manifests). Supply-chain artifacts (CycloneDX SBOM, SLSA v1.0 provenance, cosign signatures). The self-update protocol (manifest schema, channel resolution, signature verification, atomic replace, Windows rename, rollback). The v4.0.0 install surface (GitHub Releases + curl-pipe installer; Homebrew/scoop/winget/distro deferred) and side-by-side `lando4` ownership next to an untouched Lando 3 `lando`. The CI release workflow on GitHub Actions and the binary-shipping acceptance criteria that augment §15.C. |
...
| Level-`none` native fast path | 03 | §3.2 |
...
| Two-way config translation (bundled `lando4`, `recipe`, `lando3` translators; explicit-only invocation) | 04 + 07 | §4.2 + §7.4.1 |
| Authoring-IR invariant (`LandofileAuthoringShape` / `LandofileAuthoringFragment`, set input, encode round-trip law, diagnostic kinds) | 07 | §7.4.1 |
| `RecipeDecomposer` SDK port | 07 + 08 | §7.4.1 + §8.8 |
| Loader failure path (`Lando3LandofileDetected`, `LandofileDialectMixError`) | 07 | §7.1 |
| Seven-layer Lando 3 set conversion and recipe-layer folding | 07 | §7.2 |
| `@lando/lando3` bundled plugin (decode-only translator, doctor checks) | 01 + 04 + 11 | §1.4 + §4.2 + §10.9 |
...
| Planned per-step build users (`string \| { run, user? }`) | 06 | §6.2 + §6.3 + §6.13.5 |
| Mounts, excludes/includes semantics | 06 | §6.4 |
| Storage scopes and auto-naming | 06 | §6.5 |
| Default planned-user `home:` storage and `HomePathCapabilityError` | 06 | §6.5 |
| Endpoints, hostnames, routes, route filters | 06 | §6.6 |
| Route shorthand normalization and filter merge order (`stripPrefix`, `addPrefix`, headers, `redirect`) | 06 + 11 | §6.6 + §10.2 |
| Healthchecks | 06 | §6.7 |
...
| Single shipped `ServiceType` version matrix and unknown-version rejection | 06 | §6.11.2 + §6.12.1 |
| Catalog options (`solr.config.dir`, `config.server`, `node.globals`, `node.port`, `composer: { version, packages }`, Redis `password` / `persist`, Mailpit `mailFrom`, Apache `webroot`) | 06 | §6.12.1 + §6.12.5 |
...
| Top-level Landofile keys, Compose subset compatibility, config translation, and forbidden wrappers (`compose:`, `recipes:`) | 07 | §7.4 |
| Recipe provenance (`recipe: { id, version, producer, options, services? }`, inert at runtime) | 07 + 08 | §7.4 + §8.8 |
| `recipe.<option>` expression scope | 07 | §7.3.1 |
| Secret dispositions (`secret-store` reference or named init-only sink) | 07 + 08 | §7.3.1 + §8.8.5 |
...
| `appEnv` / `appLabels` bounded global maps (reserved `LANDO_*` catalog, reserved `dev.lando.*`) | 07 + 06 | §7.5 + §6.9 + §6.5 |
| `router.enabled` and `scanner:` union (`false \| { path?, okCodes?, retries?, timeout? }`) | 07 + 11 | §7.5 + §10.2 + §10.5 |
...
| Canonical Landofile serializer (`emitLandofileYaml` / `parseLandofile`, expression-aware round-trip law, provenance comment block) | 07 | §7.8.1 |
...
| `lando app config translate` command (`--from`, `--to`, `--file`, `--write`) | 08 | §8.2.1 |
| `lando app config explain` command (read-only provenance report) | 08 | §8.2.1 |
| `lando app config migrate` command (declarative snapshot edges, hunk classes, longest satisfied prefix) | 08 | §8.2.1 |
| Service-scoped `app:rebuild --service` / `app:info --service` | 08 | §8.2 |
| `pre-restart` / `post-restart` bracket events | 03 + 08 | §3.5 + §11.4 + §8.2 |
...
| Normalized tooling metadata (`user`, `disabled`, `FlagSpec` / `ArgSpec`, ordered steps, shared normalization) | 08 | §8.5.1 |
...
| Dynamic event names from resolved tooling, bounded nested recursion, fatal post-step failure | 08 | §8.5.7 |
...
| Recipe decomposition (`decompose`, everything written, expressions for values) | 08 | §8.8 |
| Recipe migrations (`migrations:` edges, declarative snapshots, pure helper allowlist, hunk schemas) | 08 | §8.8.3 |
| Recipe post-init secret sinks (`postInit.stdin`, `postInit.secretEnv.<name>`) | 08 | §8.8.8 |
...
| Default renderer implementation contract (OpenTUI substrate, split-footer live region, degradation, frame-snapshot tests) | 08 | §8.9.3 |
| Rich render events (`code.snippet`, `diff.render`, `markdown.block`) | 08 | §8.9.4 |
| Renderer panel slots (`rendererPanels:` plugin-contributed TUI panels) | 08 | §8.9.5 |
| Keymap (renderer action vocabulary, `KeymapConfig`, bindings) | 08 | §8.9.6 |
| Desktop notifications (`notify.desktop`, `@lando/notify-lando`) | 08 | §8.9.7 |
| Interactive log viewer (`app:logs --follow` TTY viewer) | 08 | §8.9.8 |
...
| Plugin-contributed config translators (decode plus optional encode; explicit-only) | 10 | §9.5 |
| Config translator / `RecipeDecomposer` / transaction / coexistence test suites | 13 | §13.1 |
...
| `RouterService` / `routerServices:` | 04 + 10 + 11 | §4.2 + §9.5 + §10.2 |
| `@lando/proxy-traefik` (contributes `routerServices: [traefik]`) | 01 + 04 + 18 | §1.4 + §4.2 + §20.10.1 |
...
| Host-proxy wire protocol (`openUrl`, `openPath`, `runLando`, `runBun`, NDJSON streaming) | 11 | §10.10.2 |
...
| `DoctorCheckContext` bounded ports (app identity, provider identity, resource inspector, path-only executable locator) | 10 + 11 | §9.5 + §10.9 |
| `lando3-leftovers` / `lando3-shadow` doctor checks | 11 | §10.9 |
...
| Managed-file transaction lifecycle (journal states, stages, digest-named backups, recovery, guard) | 12 | §12.4.1 |
...
| Side-by-side `lando4` ownership (install, update, uninstall, shellenv never touch Lando 3) | 01 + 15 | §1.4 + §17.6.2 + §17.7 |
...
| `RouterServiceTraefikGlobalAppLive` (default router realization) | 18 | §20.10.1 + §10.2 |
