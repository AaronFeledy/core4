...
| 02 | [`02-toolchain.md`](./02-toolchain.md) | §2 | Bun policies, single-binary build, performance budgets. TypeScript strict settings. The target CLI framework (one native command registry and dispatcher) plus the historical OCLIF-adoption rationale and current OCLIF-removal migration status. Effect runtime model and reference patterns. Effect Schema as the single contract language, including schema annotations, JSON Schema output, and generated reference docs. The Starlight docs toolchain. Forbidden runtime dependencies. The `@lando/core` package surface and entry-point catalog. |
