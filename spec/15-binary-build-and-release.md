...
| 13 | `oclif-manifest` | Command registry manifest | `scripts/build-oclif-manifest.ts` | Target (US-528): a single generator emitting the embedded, build-derived native command-registry manifest consumed directly by the single-dispatcher command registry, from native command registry + bundled plugin command metadata. Today: OCLIF command classes under `core/src/cli/oclif/commands/**` | Target: one embedded manifest module. Today: `core/oclif.manifest.json` plus `core/src/cli/oclif/compiled-manifest.ts` (the binary-embedded TS module) plus `core/src/cli/generated/command-ids.ts` (the generated command-id constant list) | derived |
| 14 | `command-reference` | CLI command reference | `scripts/build-command-reference.ts` | `LandoCommandSpec` registry + the compiled OCLIF manifest | `docs/reference/commands.mdx` | derived |
...
Use for: data that is small (under ~256 KB), structured, and read into JS objects. Examples: the bundled-plugin registry, schema-id index, and default global config. The command registry manifest has the same build-embedded semantics, but today's generator emits a TypeScript object-literal module alongside `oclif.manifest.json` pending US-528.

Today's command-manifest implementation uses the generated TypeScript embed module rather than reading the JSON artifact at runtime:

```ts
import { COMPILED_OCLIF_MANIFEST } from "./compiled-manifest.ts";

export const loadCompiledManifest = () => COMPILED_OCLIF_MANIFEST;
```

The target is a registry-derived native command manifest module (§8.4.1) replacing the OCLIF-named module once US-528 lands; it is not shown here because it does not exist yet.
...
- Removing a bundled plugin from `core/build.config.ts` and rebuilding today produces a binary that omits the plugin from `core/src/plugins/generated/bundled.ts` and, when it contributes a renderer, from `core/src/plugins/generated/renderers.ts`; today's OCLIF command manifest is built only from the OCLIF command tree and is unaffected. Target (US-528): the single embedded native command manifest also consumes bundled-plugin command metadata and omits the removed plugin there, without hand edits in `src/`.
