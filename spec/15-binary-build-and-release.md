...
A single command, `bun run codegen`, runs every generator in dependency order. **Target (architecture-simplicity US-505):** `bun run codegen:check` is the **one pure-drift gate**: it runs the full catalog and fails when a tracked generated output differs from the Git index or an untracked derived output fails the catalog's deterministic-regeneration and consumer-validation strategy. Ordinary `git diff --exit-code` covers tracked outputs only; ARCH-04 leaves the exact comparison strategy for untracked derived trees to US-505. The gate may also run deprecations/typecheck as companion checks, but those are not substitutes for drift. **Today:** `codegen:check` is `bun run codegen && bun run check:deprecations && bun run typecheck` — it regenerates the catalog and runs deprecation/typecheck as companion checks, but it does not yet assert drift. CI runs `codegen:check` on every PR (§13.4). **Semantic** gates stay separate: `check:guide-coverage`, `check:schema-compatibility`, `check:public-transcripts`, `check:package-dag`, and behavioral boundary rules.
