...
| **Runtime bundle manifest** | `scripts/build-runtime-bundle.ts` | `plugins/provider-lando/runtime-bundle-version` (a one-line file holding the pinned upstream runtime tag) + per-platform bundle artifacts assembled from upstream Podman + Lando helper binaries, staged under `dist/cache/runtime-bundle/` | `plugins/provider-lando/runtime-bundle-versions.json` (the static-asset manifest mapping `<host-platform>` to runtime-bundle download URL, file size, SHA-256; consumed at runtime via §17.3 mechanism A) | Build (stage 1); manual on runtime-version bump | Re-run + `git diff --exit-code`; the committed release manifest MUST be byte-stable for a given pinned tag, every `url` MUST resolve over HTTPS to a 200 with the recorded `Content-Length` and SHA-256, and the platform set MUST cover every host target listed in §13.5. In `--local` mode the script stages the bundle and emits a `file://` manifest with a locally-computed SHA-256 for §13.5 CI/dev verification instead; that output is never committed |
