...
The deprecation gate remains an independent release-orchestrator step after stage 1. On PRs, `bun run lint` enforces deprecations (`biome check . && bun run check:deprecations`), so conflicts surface before merge rather than only at release time. Failures surface as tagged errors with the offending `(kind, id, removeIn)` triplets and a remediation pointer to §18.7.

### 17.2 Codegen catalog

Codegen is the largest single source of correctness drift in a build like this, so the spec lists every generator explicitly. `scripts/codegen-catalog.ts` is the machine-readable executed catalog: it defines the 24 `CODEGEN_CATALOG` entries (id, ownership, script, workspace) that `scripts/codegen.ts` iterates via `resolveCodegenCommand`, in the exact order `bun run codegen` runs them (some outputs feed later steps). **Table A** below mirrors that catalog for humans, one row per entry, each with one ownership value. **Table B** lists build/maintenance scripts that touch generated-looking paths but are not wired into `scripts/codegen-catalog.ts` and so are outside the executed catalog. **Table C** quarantines scripts described in earlier drafts of this section that have no corresponding file under `scripts/` today; they are not implemented by this wave.

Ownership has three values: **committed-pin** (a human-reviewed, version-pinned manifest; offline invariants checked every run, live verification only at publish time), **committed-workflow** (a human-reviewed generated CI workflow file; re-run + `git diff --exit-code`), and **derived** (a pure build product; regenerated by codegen, not required as git SoT, verified by pure drift or, where gitignored, by "must regenerate cleanly + tests pass").

#### Table A — executed catalog (`scripts/codegen.ts`, execution order)

| # | id | Generator | Script | Input | Output | Ownership |
|---|---|---|---|---|---|---|
| 1 | `build-guide-scenarios` | Guide scenario tests (§19.7) | `scripts/build-guide-scenarios.ts` | `docs/src/content/docs/{guides,tutorials,how-to}/**/*.mdx`, `recipes/*/README.mdx`, colocated guide case files | `test/scenarios/generated/guides/**` (gitignored) + generated index | derived |
| 2 | `build-recipe-readmes` | Recipe README scaffold output (§19.13) | `scripts/build-recipe-readmes.ts` | `recipes/<id>/README.mdx` | `recipes/<id>/.scaffold/README.md` (strip-and-flatten of executable components into prose-only Markdown for `lando init` to copy into the user's project); MUST contain no MDX JSX, no `import` statements, and no unresolved interpolation expressions | derived |
| 3 | `bundled-plugins` | Bundled plugins index | `scripts/build-bundled-plugins.ts` | `plugins/` workspace + `core/build.config.ts` (the "ship list") | `core/src/plugins/generated/bundled.ts` (static `import` graph) plus `core/src/plugins/generated/renderers.ts` (the renderer-contributing subset) | derived |
| 4 | `mutagen-versions` | Mutagen versions manifest | `scripts/build-mutagen-versions.ts` | `plugins/file-sync-mutagen/mutagen-version` (a one-line file holding the pinned upstream Mutagen tag, e.g., `v0.18.3`) + the pinned per-tag SHA-256/size table (`MUTAGEN_CHECKSUMS`) in the script | `plugins/file-sync-mutagen/mutagen-versions.json` (the static-asset manifest mapping `<hostKey>/cli` and `<hostKey>/agent/<guest>` artifact keys to download URL, file size, SHA-256; validated against the canonical `ToolManifest` schema and consumed at runtime by the tool-provisioning helper via §17.3 mechanism A, §10.3.4). Agent entries reuse the host archive's url/sha256 so the byte cache de-duplicates the one download across host + agents | committed-pin |
| 5 | `provider-images` | Provider image pin | `scripts/build-provider-images.ts` | Pinned `{ image, digest }` table in the script (the generic-fallback `tar` helper image, `dataHelper`, and peers) | `engine/src/data-mover/generated/provider-images.ts` — resolved through `RuntimeProvider.pullArtifact`, digest-verified, cached, and offline-reused like every other Lando-provisioned artifact | derived |
| 6 | `compose-fixture-manifest` | Compose fixture rejection manifest | `scripts/build-compose-fixture-manifest.ts` | `core/test/fixtures/compose/**` (the trusted fixture landofiles) | `core/test/fixtures/compose/manifest.json` (per-fixture rejected Compose key-matrix paths) | derived |
| 7 | `bundled-recipes` | Bundled recipes index | `scripts/build-bundled-recipes.ts` | Target: `recipes/<id>/` directories. Today: `core/build.config.ts` (the "ship list") plus `core/src/recipes/builtin/<id>/manifest.ts` for each listed id | Target: `core/src/recipes/bundled.ts` (id → embedded FS handle, plus a `bunScripts:` map of `{ path → sha256 }` for every `.bun.sh` file referenced by the recipe's `postInit.bun: { verb: script }` actions, §8.8.8) plus `dist/embedded/recipes-<id>.tar` (per §17.3) — not implemented yet. Today: `core/src/recipes/bundled.ts` records a static `import` graph of `{ id, source, manifestYaml }` the compiled binary can use without dynamic `import()` and without runtime disk reads against the recipe templates; no tar, no embedded FS handle, no `bunScripts` map | derived |
| 8 | `bootstrap-layers` | Bootstrap layers | `scripts/build-bootstrap-layers.ts` | Core service registry (§3.4 service-membership-per-level table), `BootstrapLevel` enum (§3.2) / `BOOTSTRAP_RANK`, `engine/src/runtime/bootstrap-layer-support.ts` runtime-varying inputs, bundled-plugin contribution graph | `core/src/runtime/generated/layers/none.ts`, `…/minimal.ts`, `…/plugins.ts`, `…/commands.ts`, `…/tooling.ts`, `…/provider.ts`, `…/app.ts` — one prebuilt `Layer` per level with eager services constructed and lazy services wrapped in `Layer.suspend` per §2.4 | derived |
| 9 | `schema-snapshot` | Public schema artifact set | `scripts/build-schema-snapshot.ts` | Public `@lando/sdk/schema` registry, standalone result schemas for every canonical command (`core/src/cli/oclif/compiled-commands.ts`), decoded manifests for the in-binary bundled plugins only (out-of-tree plugin manifests are intentionally not discovered here) | `dist/schemas/<id>.json` (one per public schema), `dist/command-schemas/**` (one per canonical command result schema), `docs/reference/schemas/*.mdx`, `sdk/test/fixtures/bundled-plugin-manifests.json`, `core/dist/schemas/**` and `core/dist/command-schemas/**` (gitignored byte-identical package mirrors used by npm pack/publish without Git history) | derived |
| 10 | `setup-plugin-flags` | Setup plugin flags | `scripts/build-setup-plugin-flags.ts` | `core/build.config.ts` (the "ship list") + each bundled plugin's `manifest.contributes.setup.flags` | `core/src/cli/generated/setup-plugin-flags.ts` — plain literal data (no plugin/Effect imports) that the `meta:setup` command merges into its flag surface, so importing it never pulls the bundled plugin Layers into scope | derived |
| 11 | `mcp-allowlist` | MCP tool allowlist | `scripts/build-mcp-allowlist.ts` | `core/src/cli/oclif/compiled-commands.ts` (every `LandoCommandSpec` with `mcpAllowed: true`) | `core/src/cli/oclif/generated/mcp-allowlist.ts` — plain literal data listing the default set of canonical command ids `lando mcp` exposes as tools | derived |
| 12 | `host-proxy-allowlist` | Host-proxy command allowlist | `scripts/build-host-proxy-allowlist.ts` | `core/src/cli/oclif/compiled-commands.ts` (every `LandoCommandSpec` with `hostProxyAllowed: true`) | `core/src/cli/oclif/generated/host-proxy-allowlist.ts` — plain literal data listing the canonical command ids the host-proxy `runLando` dispatcher will accept | derived |
| 13 | `command-registry-manifest` | Command registry manifest | `scripts/build-command-registry-manifest.ts` | Native `builtInCommandEntries` registry metadata plus `COMMAND_TOPICS` (built-in `LandoCommandSpec`s and their topics only). Bundled- and external-plugin command metadata MUST stay out of this generator; the validated `plugin-command` cache is their sole metadata owner (§12.1, §9.7). | `core/src/cli/generated/command-registry-manifest.ts` (the binary-embedded TypeScript object-literal module) plus `core/src/cli/generated/command-ids.ts` (the generated command-id constant list); no JSON sidecar | derived |
| 14 | `command-reference` | CLI command reference | `scripts/build-command-reference.ts` | `LandoCommandSpec` registry + the embedded command-registry manifest | `docs/reference/commands.mdx` | derived |
| 15 | `compose-key-matrix` | Compose key-matrix reference | `scripts/build-compose-key-matrix.ts` | Committed Compose dispositions (`core/src/landofile/compose/dispositions.ts`) | `docs/reference/compose-key-matrix.mdx` | derived |
| 16 | `opentui-native-stubs` | OpenTUI native stub catalog | `scripts/build-opentui-native-stubs.ts` | The 8-root native-package catalog (`@opentui/core-linux-x64`, `-linux-arm64`, `-darwin-x64`, `-darwin-arm64`, `-win32-x64`, `-win32-arm64`, `-linux-x64-musl`, `-linux-arm64-musl`), the 5-entry release-target-to-native-root mapping (§17.3.1's table), the `plugins/renderer-lando` package (to confirm its `@opentui/core` dependency range), the installed `@opentui/core` package (to read its own literal `import("@opentui/core-<target>")` branch list and keep the catalog honest against what's actually on disk), and `bun.lock` (to pin exact resolved versions) | `scripts/generated/opentui-native/catalog.generated.ts` (the machine-readable 8-root/5-mapping table §17.3.1's build plugin imports) plus 35 generated stub modules under `scripts/generated/opentui-native/stubs/<target>/<redirected-root>.generated.ts` (5 targets × 7 redirected roots each = 35; every stub is import-free and its entire body throws a fixed `Error` on any access, so a redirected root can never accidentally resolve to working code). The generator fails closed if the installed `@opentui/core`'s own literal branch list no longer matches the 8-root catalog (a new/removed upstream native package is a generator-blocking drift, not a silent mismatch). Focused command: `bun run codegen:opentui-native-stubs` | derived |
| 17 | `php-base-images` | PHP base image Dockerfiles | `scripts/build-php-base-images.ts` | `plugins/service-lando/src/services/php-prerequisites.ts` (extensions, Composer pin, prerequisites command) + `plugins/service-lando/src/services/php.ts` (`SUPPORTED_PHP_VERSIONS`) | `images/php/<version>/Dockerfile` (one per supported PHP version) | derived |
| 18 | `ci-workflow` | CI workflow | `scripts/build-ci-workflow.ts` | `scripts/ci-platforms.ts` + `scripts/runtime-bundle-supply-chain.ts` | `.github/workflows/ci.yml` | committed-workflow |
| 19 | `nightly-workflow` | Nightly workflow | `scripts/build-nightly-workflow.ts` | Shared config from `scripts/build-ci-workflow.ts` | `.github/workflows/nightly.yml` | committed-workflow |
| 20 | `release-workflow` | Release workflow | `scripts/build-release-workflow.ts` | Release stage/platform config in the script | `.github/workflows/release.yml` | committed-workflow |
| 21 | `provider-matrix-workflow` | Provider matrix workflow | `scripts/build-provider-matrix-workflow.ts` | Provider matrix config in the script | `.github/workflows/provider-matrix.yml` | committed-workflow |
| 22 | `runtime-bundle-workflow` | Runtime bundle workflow | `scripts/build-runtime-bundle-workflow.ts` | Runtime-bundle release/state-machine config in the script (dispatches `build-runtime-bundle.ts`, Table B, at CI time; also triggers `provider-matrix.yml` on publish) | `.github/workflows/runtime-bundle.yml` | committed-workflow |
| 23 | `php-base-workflow` | PHP base image workflow | `scripts/build-php-base-workflow.ts` | PHP base image build/push config in the script | `.github/workflows/php-base-images.yml` | committed-workflow |
| 24 | `compose-vendor-bump-workflow` | Compose vendor bump workflow | `scripts/build-compose-vendor-bump-workflow.ts` | `scripts/compose-vendor.ts` helpers (`selectNewerComposeGoTag`, `readComposeVendorPin`) referenced by the emitted workflow | `.github/workflows/compose-vendor-bump.yml` | committed-workflow |

Ownership totals for Table A: 1 committed-pin, 7 committed-workflow, 16 derived — 24 rows total, matching `scripts/codegen.ts` exactly.

Codegen MUST be deterministic: re-running with identical inputs produces byte-identical outputs.

#### Table B — outside the executed catalog (maintenance/network scripts)

These scripts read or write paths that look generator-shaped but are **not** invoked by `scripts/codegen.ts`; `bun run codegen` and `bun run codegen:check` never run them. They're invoked directly (locally, or from a dedicated CI job) when their upstream pin needs a bump.

| Script | Purpose | Input | Output | Notes |
|---|---|---|---|---|
| `scripts/build-runtime-bundle.ts` | Runtime bundle manifest generator for `@lando/provider-lando` | `plugins/provider-lando/runtime-bundle-version` (a one-line file holding the Lando runtime-bundle version; it names the `runtime-v<version>` release tag on **this repository** whose assets the manifest references) + per-platform bundle artifacts assembled from pinned upstream Podman + Lando helper binaries, staged under `dist/cache/runtime-bundle/` | `plugins/provider-lando/runtime-bundle-versions.json` (the static-asset manifest mapping `<host-platform>` to runtime-bundle download URL, file size, SHA-256; consumed at runtime via §17.3 mechanism A) | Invoked by the `runtime-bundle-workflow` CI job (Table A row 22), not by `bun run codegen`. **Offline invariant checks (every pass, no network):** every `url` is an HTTPS URL under this repository's `releases/download/runtime-v<version>/` path, the platform set covers all four supported runtime hosts (`linux-x64`, `linux-arm64`, `darwin-arm64`, `win32-x64` — `windows-x64` maps to `win32-x64`), and no entry carries a placeholder checksum or `sizeBytes: 0` (§5.8.1 committed-manifest invariant). **Live verification (release path + periodic job, never per-PR):** every `url` resolves over HTTPS to a 200 with the recorded `Content-Length`; the recorded SHA-256 is proven at publish time, when the manifest is regenerated against the just-published assets (§17.8). In `--local` mode the script stages the bundle and emits a `file://` manifest with a locally-computed SHA-256 for §13.5 CI/dev verification instead; that output is never committed |
| `scripts/build-compose-vendor.ts` | Vendored upstream Compose spec bump | `vendor/compose/pin.json` (the current pinned upstream Compose Go module tag) + upstream Compose spec source fetched over the network at the pinned/candidate tag | `vendor/compose/pin.json` (updated pin) + `vendor/compose/compose-spec.json` (checksum-pinned upstream bytes, written verbatim; MUST NOT be reformatted) | Network-dependent (fetches upstream release metadata and source); invoked manually or by the `compose-vendor-bump-workflow` CI job (Table A row 24), never by `bun run codegen` |
| `scripts/build-compose-fixtures.ts` | Compose test fixture bump against upstream | `core/test/fixtures/compose/pin.json` (the current pinned upstream fixture commit) + upstream fixture sources fetched over the network at the pinned/candidate commit | `core/test/fixtures/compose/pin.json` (updated pin) + refreshed fixture files under `core/test/fixtures/compose/**` (which then feed `compose-fixture-manifest`, Table A row 6, on the next `bun run codegen`) | Network-only: fetches fixture sources from the upstream commit. Invoked manually, never by `bun run codegen` |

#### Table C — planned, not yet implemented

These generators were described in earlier drafts of this section. No corresponding script exists under `scripts/` today; they are not implemented by this wave. Their target inputs/outputs are recorded here so a later wave doesn't have to re-derive them, and so this table stays the one place old cross-references resolve to instead of a stale row in Table A.

| Planned id (future) | Purpose | Target input | Target output |
|---|---|---|---|
| `build-error-docs` (`scripts/build-error-docs.ts`) | Error catalog docs | Tagged-error class registry (`src/errors/**`) | `docs/src/content/docs/reference/errors/*.mdx` |
| `build-event-docs` (`scripts/build-event-docs.ts`) | Event catalog docs | Event payload schemas (`src/lifecycle/schema.ts`) | `docs/src/content/docs/reference/events/*.mdx` |
| `build-types` (`scripts/build-types.ts`) | TypeScript declaration bundling | `src/**/*.ts` (wraps `tsc -p tsconfig.types.json`) | `dist/types/<entry>.d.ts` per §2.7 entry |
| `build-api-reports` (`scripts/build-api-reports.ts`) | Public API reports (API Extractor or equivalent) | `package.json#exports` + public entry points | `etc/api/*.api.md` |
| `build-service-docs` (`scripts/build-service-docs.ts`) | Service registry docs | Core service tag registry (§3.4) + pluggability catalog (§4.2) | `docs/src/content/docs/reference/services/*.mdx` |
| `build-recipe-action-docs` (`scripts/build-recipe-action-docs.ts`) | Recipe action docs | Recipe action registry + command metadata (`recipePostInitAllowed`) | `docs/src/content/docs/reference/recipes/actions.mdx` |
| `build-acceptance-index` (`scripts/build-acceptance-index.ts`) | Acceptance coverage index | Acceptance checklist ids + test metadata | `dist/acceptance-index.json` |
| `build-mutagen-client` (`scripts/build-mutagen-client.ts`) | Mutagen gRPC client codegen | `plugins/file-sync-mutagen/vendor/mutagen-protos/**/*.proto` (vendored at the pinned Mutagen version) + `plugins/file-sync-mutagen/mutagen-versions.json` | `plugins/file-sync-mutagen/src/generated/**/*.ts` (Connect-ES TypeScript client and message types for the Mutagen `Synchronization`, `Daemon`, and `Prompting` services) |
| `build-bundled-plugin-templates` (`scripts/build-bundled-plugin-templates.ts`) | Bundled plugin templates index | `plugin-templates/<id>/` directories (the §9.10.2 template set: `provider`, `service-type`, `tooling-engine`, `template-engine`, `proxy`, `ca`, `recipe`, etc.) | `src/plugin-templates/bundled.ts` (template id → embedded FS handle plus the `BunSelfRunner.create`-compatible spec) plus `dist/embedded/plugin-template-<id>.tar` (per §17.3) |

**Committed pins vs derived outputs (architecture-simplicity US-501+).** Pin manifests (runtime-bundle versions, mutagen versions, compose vendor pin) and human-reviewed generated CI workflows stay in git with offline invariant checks. Pure build products — including `dist/schemas/**`, `dist/command-schemas/**`, generated schema/command reference MDX where applicable, bake-time bootstrap/plugin tables, and the command registry embed manifest — are **derived**: produced by codegen, **not** required as git SoT, still embedded into npm packs and the compiled binary at build time. Guide-scenario outputs and transcripts remain gitignored and validated by "must regenerate cleanly + tests pass" (§19.7, §19.6) — that model is the template for other derived trees. The Table A ownership column above is the human-readable form of this split; `scripts/codegen-catalog.ts` is the current machine-readable classification (its `CODEGEN_CATALOG` entries map each id to its ownership value, and `scripts/codegen.ts` consumes it directly rather than declaring ownership inline).

A single command, `bun run codegen`, runs every generator in dependency order and remains the command to reach for while iterating locally. **Shipped (architecture-simplicity US-505):** `bun run codegen:check` is the **one pure-drift gate**: it runs `bun run codegen && bun run check:codegen-drift`. The drift check fails when a generated output differs from the Git index or a new untracked output appears. Catalog path ownership is named by `CATALOG_OUTPUT_PATHS` in `scripts/check-codegen-drift.ts`, and the drift check uses pathspec-scoped `git status --porcelain=v1 --untracked-files=all`, so unrelated dirty files elsewhere in the working tree don't false-fail it. Gitignored derived trees stay outside that git-status half; they're covered instead by their existing generator/consumer validation. Deprecation enforcement on PRs is `bun run lint`; typecheck stays a separate gate. CI runs `codegen:check` on every PR (§13.4). **Semantic** gates stay separate: `check:guide-coverage`, `check:schema-compatibility`, `check:public-transcripts`, `check:package-dag`, and behavioral boundary rules.
...
Use for: data that is small (under ~256 KB), structured, and read into JS objects. Examples: the bundled-plugin registry, schema-id index, and default global config. The command registry manifest has the same build-embedded semantics and is generated as a TypeScript object-literal module with no JSON sidecar.

The command-manifest implementation statically imports the generated TypeScript module, so Bun embeds it without a runtime filesystem read:
...
That module is the registry-derived native command manifest (§8.4.1); it replaced the OCLIF-named manifest module and its JSON sidecar.

Bun inlines direct JSON imports as parsed objects and the generated command-manifest module as an object literal. Neither form performs a filesystem read at runtime; both preserve static typing for consumers.

**Mechanism B — `Bun.embeddedFiles` (binary blobs and large directory trees).**

Use for: data that is large (over ~256 KB), or that is a tree of files preserving file shape, or that is binary (templates, assets, fragments, archives). Target examples include each canonical recipe's directory tree, large schema bundles, and embedded help PDF/MDX if any.
...
**Target (architecture-simplicity, not implemented yet).** The generator (`scripts/build-bundled-recipes.ts`) is meant to produce:

- `core/src/recipes/bundled.ts` — a tiny TS module mapping `<recipe-id>` → `{ archive: <embedded handle>, manifest: <inlined recipe.yml object> }`.
...
Under that target, the recipe loader is the only consumer that reads from these handles. It exposes a virtual filesystem API (§8.8) so `lando init --recipe <id>` can iterate, render, and write recipe files without a disk read on the source side.

**Today.** `scripts/build-bundled-recipes.ts` emits only `core/src/recipes/bundled.ts`, a static array of `{ id, source, manifestYaml }` records built from `core/build.config.ts`'s ship list and each id's `core/src/recipes/builtin/<id>/manifest.ts`. There is no per-recipe tar, no embedded FS handle, no `bunScripts` map, and no virtual filesystem; `manifestYaml` is the recipe manifest embedded as a plain string constant, and there is no directory tree or file-by-file access.
...
- The compiled binary's `import()` graph contains no path that performs a runtime filesystem read of bundled plugins, bundled recipes, the native command-registry manifest, or any built-in schema. (Asserted by the import-boundary test in §13.4.)
...
- `bun run codegen:check` succeeds on a clean checkout, and fails when a tracked catalog output drifts or a new untracked output appears under a catalog path named by `CATALOG_OUTPUT_PATHS` in `scripts/check-codegen-drift.ts`; gitignored derived trees stay outside that git-status half and rely on their existing generator/consumer validation.
- Removing a bundled plugin from `core/build.config.ts` and rebuilding omits the plugin from `core/src/plugins/generated/bundled.ts` and, when it contributes a renderer, from `core/src/plugins/generated/renderers.ts`. The validated `plugin-command` cache is the sole metadata owner for bundled and external plugin commands; regenerating the bundled table and rewriting that cache omits a removed plugin's command ids without hand edits under `core/src/`. The embedded native command-registry manifest remains generated only from built-in `builtInCommandEntries`. Tests MUST lock same-name source precedence across every implemented §9.3 source, deterministic first-occurrence deduplication of duplicate command ids, order-sensitive write/read freshness symmetry, bundled-default symmetry, explicit invalidation, and the isolated ship-list removal proof. The specified experimental source has no runtime implementation yet and MUST use the same cache path when implemented. Router consumption and executable plugin-command dispatch remain deferred for the contract reasons in §9.7.
...
- Adding a new canonical recipe today requires listing it in `core/build.config.ts`'s ship list and adding `core/src/recipes/builtin/<id>/manifest.ts` (no other `src/` code change); running `bun run codegen` regenerates `core/src/recipes/bundled.ts` and the recipe is reachable via `lando init --recipe <id>` in the next built binary. (Target, not implemented yet: dropping a `recipes/<id>/recipe.yml` directory alone is sufficient, per §17.2's Bundled recipes index row.)
