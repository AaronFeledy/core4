...
| 13 | `command-registry-manifest` | Command registry manifest | `scripts/build-command-registry-manifest.ts` | Native `builtInCommandEntries` registry metadata plus `COMMAND_TOPICS` (built-in `LandoCommandSpec`s and their topics only). Bundled- and external-plugin command metadata stay out of this generator; they are owned by the validated `plugin-command` cache (§12.1, §9.7). Whether any bundled-plugin command metadata is ever also embedded here is **follow-up US-534**, not US-528. | `core/src/cli/generated/command-registry-manifest.ts` (the binary-embedded TypeScript object-literal module) plus `core/src/cli/generated/command-ids.ts` (the generated command-id constant list); no JSON sidecar | derived |
...
- The compiled binary's `import()` graph contains no path that performs a runtime filesystem read of bundled plugins, bundled recipes, the command registry manifest (today's OCLIF manifest), or any built-in schema. (Asserted by the import-boundary test in §13.4.)
...
- Removing a bundled plugin from `core/build.config.ts` and rebuilding today produces a binary that omits the plugin from `core/src/plugins/generated/bundled.ts` and, when it contributes a renderer, from `core/src/plugins/generated/renderers.ts`. **US-528 (landed):** the embedded native command-registry manifest is generated only from built-in `builtInCommandEntries` and replaces the OCLIF sidecar; it does **not** embed bundled-plugin command metadata. Plugin commands (bundled and external) remain owned by the validated `plugin-command` cache (§12.1, §9.7). **Follow-up (US-534):** decide and implement bundled-versus-external plugin command-metadata ownership — including whether any bundled-plugin command metadata is also embedded and consumed directly by the native dispatcher, precedence/invalidation against the `plugin-command` cache, generated inputs, and a remove-a-bundled-plugin proof for command metadata — without hand edits in `src/`.
