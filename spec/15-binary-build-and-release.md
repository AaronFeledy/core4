...
| 10 | `setup-plugin-flags` | Setup plugin flags | `scripts/build-setup-plugin-flags.ts` | `core/build.config.ts` (the "ship list") + each bundled plugin's `manifest.contributes.setup.flags` | `core/src/cli/oclif/generated/setup-plugin-flags.ts` — plain literal data (no plugin/Effect imports) that the `meta:setup` command merges into its flag surface, so importing it never pulls the bundled plugin Layers into scope | derived |
...
| 13 | `command-registry-manifest` | Command registry manifest | `scripts/build-command-registry-manifest.ts` | Native `builtInCommandEntries` registry metadata only (built-in `LandoCommandSpec`s). Bundled- and external-plugin command metadata stay out of this generator; they are owned by the validated `plugin-command` cache (§12.1, §9.7). Whether any bundled-plugin command metadata is ever also embedded here is **follow-up US-534**, not US-528. | `core/src/cli/generated/command-registry-manifest.ts` (the binary-embedded TypeScript object-literal module) plus `core/src/cli/generated/command-ids.ts` (the generated command-id constant list); no JSON sidecar | derived |
