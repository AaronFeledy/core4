...
Codegen is the largest single source of correctness drift in a build like this, so the spec lists every generator explicitly. Each generator has a single input root, a single output root, and a classification as either a **committed pin** (human-reviewed SoT; offline invariants) or a **derived output** (regenerated by codegen; not required in git; verified by pure drift).
...
| **Command registry manifest** | `scripts/build-oclif-manifest.ts` successor / registry embed generator (architecture-simplicity US-528; may repurpose or replace the OCLIF-named script) | Native command registry + bundled plugin command metadata | Registry-derived embed manifest (not a shipping OCLIF runtime artifact) | Build (stage 1); pre-test | Derived: covered by `codegen:check` |
...
A single command, `bun run codegen`, runs every generator in dependency order. A single command, `bun run codegen:check`, is the **one pure-drift gate**: it runs the full catalog and fails if any catalog output path drifts (`git diff --exit-code` on the catalog set, including untracked derived files that should match generators). It may also run deprecations/typecheck as companion checks, but those are not substitutes for drift. CI runs `codegen:check` on every PR (§13.4). **Semantic** gates stay separate: `check:guide-coverage`, `check:schema-compatibility`, `check:public-transcripts`, `check:package-dag`, and behavioral boundary rules.
