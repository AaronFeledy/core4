...
- `bun run codegen:check` succeeds on a clean checkout. Once US-505 lands, it also fails when any tracked or untracked catalog output drifts under the catalog's documented comparison strategy; until then, tracked generated paths require a separate `git diff --exit-code` check and untracked derived outputs rely on their generator/test validation.
