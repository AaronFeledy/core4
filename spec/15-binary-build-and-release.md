...
Codegen is the largest single source of correctness drift in a build like this, so the spec lists every generator explicitly. Each generator has explicit input and output paths plus a classification as either a **committed pin** (human-reviewed SoT; offline invariants) or a **derived output** (regenerated by codegen; not required in git; verified by pure drift).

| Generator | Script | Input | Output | When it runs | Staleness gate |
|---|---|---|---|---|---|
| **Bundled plugins index** | `scripts/build-bundled-plugins.ts` | `plugins/` workspace + `core/build.config.ts` (the "ship list") | `core/src/plugins/generated/bundled.ts` (static `import` graph) plus `core/src/plugins/generated/renderers.ts` (the renderer-contributing subset) | Build (stage 1); dev watch on plugin add/remove | Re-run + `git diff --exit-code` |
...
| **Command registry manifest** | Target (US-528): a single generator emitting the embedded, build-derived native command-registry manifest consumed directly by the single-dispatcher command registry. Today: `scripts/build-oclif-manifest.ts` builds from the OCLIF command tree | Target: native command registry + bundled plugin command metadata. Today: OCLIF command classes under `core/src/cli/oclif/commands/**` | Target: one embedded manifest module. Today: `core/oclif.manifest.json` plus `core/src/cli/oclif/compiled-manifest.ts` (the binary-embedded TS module) plus `core/src/cli/generated/command-ids.ts` (the generated command-id constant list) | Build (stage 1); pre-test | Derived: target covered by the US-505 pure-drift `codegen:check` gate (§17.2) |
...
| **CLI command reference** | `scripts/build-command-reference.ts` | `LandoCommandSpec` registry + OCLIF manifest | `docs/reference/commands.mdx` | Build (stage 5); docs build | Same |
