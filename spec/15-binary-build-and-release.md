...
| 5 | `provider-images` | Provider image pin | `scripts/build-provider-images.ts` | Pinned `{ image, digest }` table in the script (the generic-fallback `tar` helper image, `dataHelper`, and peers) | `data-mover/src/generated/provider-images.ts` — resolved through `RuntimeProvider.pullArtifact`, digest-verified, cached, and offline-reused like every other Lando-provisioned artifact | derived |
