...
| 5 | `provider-images` | Provider image pin | `scripts/build-provider-images.ts` | Pinned `{ image, digest }` table in the script (the generic-fallback `tar` helper image, `dataHelper`, and peers) | `core/src/data-mover/generated/provider-images.ts` — resolved through `RuntimeProvider.pullArtifact`, digest-verified, cached, and offline-reused like every other Lando-provisioned artifact | derived |
...
| 8 | `bootstrap-layers` | Bootstrap layers | `scripts/build-bootstrap-layers.ts` | Core service registry (§3.4 service-membership-per-level table), `BootstrapLevel` enum (§3.2) / `BOOTSTRAP_RANK`, `core/src/runtime/bootstrap-layer-support.ts` runtime-varying inputs, bundled-plugin contribution graph | `core/src/runtime/generated/layers/none.ts`, `…/minimal.ts`, `…/plugins.ts`, `…/commands.ts`, `…/tooling.ts`, `…/provider.ts`, `…/app.ts` — one prebuilt `Layer` per level with eager services constructed and lazy services wrapped in `Layer.suspend` per §2.4 | derived |
