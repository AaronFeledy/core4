...
**Concurrent meta/infra wave (not a feature-surface expansion):** **Architecture simplicity** (`spec/architecture-simplicity/`, US-500..US-532) may land during Beta 1. It collapses dual CLI dispatch to one native engine, stops committing pure derived artifacts, promotes private package seams (`@lando/state-store` + DAG), and strengthens `codegen:check` as the single pure-drift gate. It does **not** add end-user commands or Landofile keys and does **not** license reopening feature freeze for product surface. Private internal workspace packages are allowed under §2.7 without splitting public `@lando/core` into runtime+CLI publish packages. ~~Further extractions (network, managed-file, planner splits) remain deferred.~~ **Superseded for the Landofile and planner/runtime-engine seams by `spec/core-seams/`. Superseded for the managed-file package seam, in-engine planner/update module decomposition, drift-only `codegen:check`, and `check:boundaries` alias surface by `spec/scanner-retirement/` (US-551..US-560). Network and other remaining engine directory extractions (e.g. data-mover, scratch-app as packages) remain deferred.**

**Concurrent L3-parity wave (feature-surface completion before freeze):** **Lando 3 parity** (`spec/lando3-parity/`, US-561..US-581) closes the audited Lando 3 → 4 functionality gaps before feature freeze: un-gating the four spec'd-but-rejected Landofile keys (`events:`, `toolingDefaults:`, `commandAliases:`, top-level `env_file:`), completing the §6.12.1 canonical service catalog (rabbitmq, minio, localstack, mailpit/mailhog app-scoped types, plus the amended tomcat, varnish, dotnet, mssql, phpmyadmin entries), PHP service depth (§6.12.5: 8.5, serving modes, Composer options, Xdebug, `db_client`), Compose `configs:` provider realization, recipe composition (§8.8.15) and the L3-parity recipes (laravel, symfony, backdrop, joomla, mean) with option parity (§8.8.16), and the bundled `@lando/sql` helpers (§10.7, pulled forward from 4.1). Hoster connectors (`RemoteSource`, 4.1), custom SSH-key mounts (rejected, §10.4), and the remaining §8.8.10 staged recipes stay out of scope.
...
- Additional canonical recipes based on adoption signal (Magento, Astro, Next.js, django, rails, static-site generators, etc. — the §8.8.10 staged list). The L3-parity set (laravel, symfony, backdrop, joomla, mean) plus recipe composition (`extends:`, §8.8.15) landed in Beta 1 via `spec/lando3-parity/`
...
- **Compose service-key vocabulary** (`spec/compose/` PRDs, US-466..US-477): any Compose service definition pastes under `services.<name>` and works — Compose spellings plus short/long forms normalize into the plan (§6.2), the per-container runtime-knob tier rides `ServicePlan.extensions.compose` behind provider capabilities (§5.5.1), project-composition machinery (`extends`, `container_name`, `network_mode`, `links`, override tags) is rejected with remediation, and alignment with the upstream spec is held mechanically by the vendored compose-go schema pin + committed disposition matrix + `check:compose-coverage` gate (§7.4)
