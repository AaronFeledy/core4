...
- First consumers of the Beta-1 `DataMover` primitive (§10.11): a bundled `@lando/sql` plugin (`db import/export/snapshot/restore/reset` with gzip + progress) and `image save`/`load`, both built entirely on `DataMover` + the provider data plane; the `HostingProvider` pull/push abstraction (Pantheon/Acquia/Platform.sh — remote half via `HttpClient`, local landing half via `DataMover`) lands here too. These are feature work on top of the primitive, not new primitives
