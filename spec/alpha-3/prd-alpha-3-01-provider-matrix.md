# PRD: ALPHA3-01 — Provider matrix complete

## Introduction

This PRD covers Phase 3 Alpha 3 work for the **runtime-provider matrix**. Alpha 1 brought `@lando/provider-lando` to Linux + macOS and `@lando/provider-docker` to Linux + macOS. Alpha 3 finishes the picture: Windows for both, the opt-in `@lando/provider-podman`, and one shared contract suite that every provider passes on every platform it declares support for.

Depends on: **—** (Alpha 3 entry point).
...
- Code signing / notarization of provider runtime bundles (Beta 1).
...
- Provider trust / signing model for community providers (Beta 1).
