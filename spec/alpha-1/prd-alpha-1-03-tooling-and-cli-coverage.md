# PRD: ALPHA1-03 — Tooling and CLI coverage

## Introduction

This PRD covers Phase 2 Alpha 1 work for **Tooling and CLI coverage**. It translates the Alpha 1 section of [`spec/ROADMAP.md`](../ROADMAP.md) into implementation-sized stories while preserving the MVP rule that the detailed spec parts remain source of truth.
...
- Parse and execute Alpha 1 `tooling:` definitions.
- Implement providerExec and host tooling engines.
- Fill Alpha 1 app/meta command coverage while preserving structured remediation for deferred commands.

## User Stories

### US-017: Parse Alpha 1 `tooling:` Landofile section

**Description:** As a user, I can define basic tooling commands in `.lando.yml`.

**Acceptance Criteria:**
- [ ] Landofile parser tests cover `tooling:` tasks with `cmds:`, `service:`, `description:`, and Alpha 1 `vars:` forms: literal value, `default:`, `sh:`, and `prompt:` per §8.5.3; remote vars and unsafe `raw:` interpolation are deferred
- [ ] Unsupported Alpha-3-only tooling features fail with remediation, not silent ignore
...
- [ ] Host mode is available for `lando shell` in Alpha 1; service shell remains Alpha 3
...
- [ ] Hot-path optimization is explicitly not required in Alpha 1
- [ ] Tests pass
- [ ] Typecheck passes
- [ ] Lint passes
...
**Description:** As a user, I can execute commands, use Alpha 1 `ssh` as provider-exec TTY command behavior, and open host-mode shells from the CLI.
...
- [ ] `shell` host mode works; service shell and SSH subsystem/sidecar features return NotImplemented/Alpha 3 remediation
...
### US-023: Fill Alpha 1 app command coverage
...
### US-024: Fill Alpha 1 meta/app management commands
...
- [ ] Trust decisions are in-memory for the current install only — no persistent trust store in Alpha 1; a persistent trust store is deferred to Alpha 3 and a test asserts the prompt fires with a fresh `<userConfRoot>` per test invocation
