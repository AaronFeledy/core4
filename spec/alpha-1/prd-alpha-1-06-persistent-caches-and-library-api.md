# PRD: ALPHA1-06 — Persistent caches and library API

## Introduction

This PRD covers Phase 2 Alpha 1 work for **Persistent caches and library API**. It translates the Alpha 1 section of [`spec/ROADMAP.md`](../ROADMAP.md) into implementation-sized stories while preserving the MVP rule that the detailed spec parts remain source of truth.
...
- Move MVP in-memory/cache assumptions to persistent Alpha 1 caches.
...
**Description:** As a maintainer, all Alpha 1 caches use one encoding policy and compatibility checks.
...
- [ ] TestRuntimeProvider passes provider contract suite for Alpha 1 features it claims
...
**Description:** As an embedding host, I can invoke CLI commands programmatically for the Alpha 1-supported surface through the Effect-bearing operations subpath.
...
**Description:** As SDK consumers, Alpha 1 can add schemas/tags but cannot mutate MVP shapes.

**Acceptance Criteria:**
- [ ] Snapshot tests compare MVP-exported schema names/tag method signatures against a frozen list
- [ ] New Alpha 1 additions are additive and documented
