...
4. Check provider capabilities before any provider action. For each service, collect from `ServicePlan.extensions.compose` (a) the preserved per-container runtime-knob paths, (b) the preserved service-level field families (`networks`, `configs`, `secrets`, `profiles`, `labels`), and (c) any matrix-`preserved` exact paths listed by `ComposePreservedPathKey` that are present as descendants (today `depends_on.*.restart` and `healthcheck.start_interval`). A knob is supported only when the selected provider declares `composeSpec: native` and includes the exact path in `composeKnobs.supported`; omitting `composeKnobs` means no knob support. A service-level field family is supported only when the selected provider declares `composeSpec: native` and includes the family in `composeServiceFields.supported`; omitting `composeServiceFields` means no service-level field support. A `ComposePreservedPathKey` path is supported only when the selected provider declares `composeSpec: native` and includes the exact path in `composePreservedPaths.supported`; omitting `composePreservedPaths` means no support for those paths. An unsupported knob, field, or preserved path fails planning with `CapabilityError` carrying `service`, `key` (the exact path or family), `providerId`, `capability: "composeSpec"`, and actionable remediation to remove the key, choose a supporting provider, or move provider-specific intent under `providers.<id>`.
