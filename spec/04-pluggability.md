...
| **Config translation** | `ConfigTranslator` | Translate external config files into Landofile fragments for preview or application | None bundled by default | Plugin contributes `configTranslators:`. Invoked explicitly through `lando app config translate`; never runs during normal app startup. Implementations MUST pass `makeConfigTranslatorContractSuite` / `runConfigTranslatorContractSuite` (§13.1). |
