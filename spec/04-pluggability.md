...
| **CLI framework** | `CommandFramework` | Argv parsing, manifest, help, plugin install commands, namespace-to-topic mapping, top-level alias registration (§8.1.1, §8.1.2) | Native command registry and dispatcher (§8.4.1); today’s code still ships the OCLIF adapter as the source-path implementation while architecture-simplicity US-522..US-531 completes the migration | Replaceable but not recommended. Building a `@effect/cli` adapter is documented. |
