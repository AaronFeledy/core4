...
| **Console logging** | `Logger` | Structured diagnostic log events with annotations | Built-in Effect logger: quiet (`none`) unless `--log-level` / `LANDO_LOG_LEVEL` / `config.logLevel` is set. Pretty on a TTY stderr, JSON otherwise. | Not a bundled plugin. Selected by `--log-level`, `LANDO_LOG_LEVEL`, or `logLevel:`. `--debug` raises the floor to `debug`. Plugins may still contribute `loggers:`. |
...
| **Config translation** | `ConfigTranslator` | Two-way translation: `detect`, `translate` (decode one ordered document set or one recipe request into authoring fragments), and optional `encode` (§7.4.1) | Bundled `lando4` (decode + encode), `recipe` (decode through the injected `RecipeDecomposer` port; no encode), and `lando3` (decode-only, contributed by bundled `@lando/lando3`) | Plugin contributes `configTranslators:`. Invoked only by explicit `app:config:translate`, `app:config:explain`, `app:config:migrate`, or `apps:init` requests; never at bootstrap, discovery, normal loading, `start`, or tooling hot paths. Implementations MUST pass `makeConfigTranslatorContractSuite` / `runConfigTranslatorContractSuite` (§13.1). |
| **CLI framework** | `CommandFramework` | Argv parsing, manifest, help, plugin install commands, namespace-to-topic mapping, top-level alias registration (§8.1.1, §8.1.2) | Native command registry and dispatcher shared by source and compiled entries (§8.4.1); OCLIF is development-only | Replaceable but not recommended. Building a `@effect/cli` adapter is documented. |
...
| **Router / routing** | `RouterService` | Realize `RoutePlan`s into running ingress | `@lando/proxy-traefik` (package name unchanged; contributes `routerServices: [traefik]`) | Plugin contributes `routerServices:`. |
...
| **Doctor diagnostics** | `DoctorService` | Run host/app/provider diagnostics and expose automated or manual remediations; checks receive a `DoctorCheckContext` (§10.9) | Built-in core checks for app config and selected-provider basics | Plugin contributes `doctorChecks:` for additional issue coverage. Implementations MUST pass `makeDoctorCheckContractSuite` / `runDoctorCheckContractSuite` (§13.1). |
| **Host proxy** | `HostProxyService` | Per-app container→host RPC dispatch: open URLs in the host browser, route in-container `lando` shim calls back into the host runtime (§10.10) | Built-in `HostProxyServiceLive` (Bun-served HTTP/JSON over a per-app Unix socket) | Plugin contributes `hostProxyServices:`. Replaceable for headless CI builds (swallow URL opens), audited builds (mandatory transcript), or remote-host transports. Selection follows §4.3. |
| **Plugin source** | `PluginSource` | Resolve and fetch a plugin spec | Built-in: registry (Bun), git, local, tarball | Plugin contributes `pluginSources:` for private registries, GitLab, etc. Implementations MUST pass `makePluginSourceContractSuite` / `runPluginSourceContractSuite` (§13.1). |
...
| **Route filter** | `RouteFilter` | Provider-neutral request/response transforms | Built-ins: `requestHeader`, `responseHeader`, `redirect`, `rewritePath`, `stripPrefix`, `addPrefix` | Plugin contributes `routeFilters:`. Implementations MUST pass `makeRouteFilterContractSuite` / `runRouteFilterContractSuite` (§13.1). |
...
| **Secret store** | `SecretStore` | Resolve `${secret:...}` references in Landofiles | Built-in env-var store | Plugin replaces with Vault, 1Password CLI, AWS SM, etc. Implementations MUST pass `makeSecretStoreContractSuite` / `runSecretStoreContractSuite` (§13.1). |
