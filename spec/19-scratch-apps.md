...
- The default (no `--detach`) is **foreground**. The CLI's `run()` body acquires the scratch under the native dispatcher's command scope and blocks until the user signals exit (Ctrl+C, `lando scratch stop` from another shell, or process termination). Scope finalization on exit destroys the scratch.
