...
| `plugin-command` | `<userCacheRoot>/plugin-command-cache.bin` | binary | Sole metadata owner for bundled and external plugin commands from validated, precedence-merged manifests; no command implementation modules | Plugin install/remove/update, regenerated bundled ship list, plugin cache invalidation, `--clear` |
...
| `command-registry-manifest` | Embedded in the binary as `core/src/cli/generated/command-registry-manifest.ts`, build-derived by `scripts/build-command-registry-manifest.ts` (§17.2) | generated TypeScript object-literal module; compiled into the binary; not written by `CacheService` | Native command-registry metadata projected from every built-in `LandoCommandSpec`; plugin commands stay in the separate `plugin-command` cache above, not in this manifest | Rebuild only (`bun run codegen`); there is no runtime write path for this artifact |
...
| `mcp-allowlist` | `<userCacheRoot>/mcp-allowlist.bin` | binary | Generated default allowlist of canonical command ids `lando mcp` exposes as MCP tools (§10.14). Built from every `LandoCommandSpec` with `mcpAllowed: true` (§8.3), including plugin commands with the same flag. Destructive surfaces (`app:destroy`, `apps:poweroff`, `meta:uninstall`, plugin mutations) are never in this default set; the effective tool set layers `mcp.allow`/`--allow` and `mcp.deny`/`--deny` on top (§8.2.6) | Plugin install/remove/update, app command index change, `--clear` |
...
**Plugin-command metadata ownership.** Bundled and external plugin command metadata MUST follow the same cache-only path. The implemented contribution graph follows the §9.3 order through bundled, system, user, app-local `pluginDirs:`, and explicit Landofile `plugins:`; a later source replaces an earlier plugin with the same name. The specified experimental source has no runtime implementation yet and MUST join this cache path at the final §9.3 precedence when implemented. Cache compilation deterministically deduplicates duplicate command ids by retaining their first occurrence in the merged manifest order, then sorts the final index by id. Writes and freshness reads MUST compare the ordered plugin-name list and derive the manifest fingerprint, plugin-list fingerprint, and per-plugin command-id map from the same resolved manifests. The build-embedded `command-registry-manifest` MUST remain built-in-only and MUST NOT duplicate plugin metadata. Because the current public plugin contract supplies only command ids and no implementation loader, router consumption and executable plugin-command dispatch remain deferred (§9.7).
...
- **Binary encoding for every cache on a hot path.** Every cache read by the router phase (`core-command`, `plugin-command`, `app-command`, `cwd-app-map`) and the tooling hot path (`app-plan`) MUST use a binary format. The build-embedded TypeScript command-registry manifest (§17.2) is also read on the router phase but is a build artifact rather than a `CacheService`-managed runtime cache. JSON is reserved for caches that are not on a hot path and that a maintainer might `cat` while debugging (`plugin`, `app-plugin`, `service-info`, `provider`, `update`).
...
| Transaction journal under the existing state-store root | Owner-only complete before/after plan and recovery state, keyed by canonical app-root identity. Explicit exception to app-root containment; lifecycle below |
| `<target>.lando-stage.<transaction-id>` | Exclusive owner-only same-directory stage recording byte digest and intended output mode; removed only by verified transaction cleanup |
| `<file>.bak.<full-before-sha256>` | Immutable backup for an existing regular input; retained until the user removes it, reused only after exact digest and owner-safe mode verification |

#### 12.4.1 Managed-file transaction lifecycle

Explicit conversion and migration (§7.4.1, §8.2.1) MUST share a private transaction module owned by `@lando/managed-file`. It uses `ManagedFileService`/`FileSystem` for user-file operations and `StateStore` (§12.7) for the journal and canonical app-root lock. Commands and translators MUST NOT bypass those services. The coordinator MUST resolve canonical root identity, reject source/destination symlinks, and require every app-file, backup, and stage parent realpath to remain contained under that root. Only the journal under the existing state-store root is exempt from app-root containment. This uses existing package seams and adds no boundary scanner.

Every existing regular input MUST have an immutable `<file>.bak.<full-before-sha256>` backup. Reuse requires a regular non-symlink backup with the exact recorded digest and owner-safe mode; any mismatch MUST fail before mutation. An absent before-state has no backup. Exclusive same-directory `<target>.lando-stage.<transaction-id>` stages MUST record digest and mode. Existing nonsecret outputs preserve mode, new outputs use a restrictive default, and secret outputs MUST be owner-only. Raw bytes, backups, and journal data MUST NOT reach renderer events, diagnostics, telemetry, or transcripts (§3.5, §3.7). Raw secret prompt answers MUST NOT enter the transaction.

The owner-only journal follows `prepared` → `committing` → `committed` or `blocked`. After creating or verifying all backups and stages, and before any target mutation, the coordinator MUST fsync a complete `prepared` journal containing the ordered before/after plan: path absence/presence, hashes, modes, backup identity/existence, stage identity/hash, and removals. Commit records `committing`, performs per-file renames with immediate digest rechecks, and MUST durably fsync `committed` before cleanup. Recovery MUST determine applied renames from before/after digests, including a crash after rename but before any applied flag. Recovery of `committed` MUST perform cleanup only.

Before recovering an incomplete transaction, the coordinator MUST preflight every lock and path against the complete recorded before/after state, including expected absence, symlink status, hashes, modes, backups, and stages. A conflict MUST preserve all user files, mark the journal `blocked`, and require manual resolution. Recovery MUST NOT overwrite a detected concurrent edit either forward or backward. Cooperative canonical-root locking plus digest rechecks is not CAS and does not provide filesystem-wide atomicity against noncooperative editors.

A crash before `prepared` MAY leave stages but MUST have changed no target. Later transactions MUST use a new id, reuse only hash-verified immutable backups, and MUST NOT guess ownership or automatically delete unjournaled stages. Manual cleanup requires the user to establish that no transaction is active, inspect the unjournaled stage, and remove only that confirmed orphan; its filename alone is not ownership evidence. Scoped cancellation removes only the current process's own stages. Prepared-journal recovery removes only stages whose recorded identity and hash match. Committed journals MUST be removed after cleanup completes; immutable backups remain until the user removes them.

A thin transaction-guard service MUST be consulted by native loading and `start` before reading a possibly partial file set. It checks the app-root journal and performs guarded recovery or refuses a `blocked` transaction without loading translators. Dry-run MUST report pending recovery but MUST NOT acquire a write lock or mutate stages, journals, backups, targets, or deletions.
