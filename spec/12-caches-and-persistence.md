...
| `command-registry-manifest` | Target: embedded in binary, build-derived from the compiled command registry. Current implementation: `core/oclif.manifest.json` + `core/src/cli/oclif/compiled-manifest.ts`, both generated at build time by `scripts/build-oclif-manifest.ts` (§17.2) | generated JSON + TypeScript embed module; compiled into the binary; not written by `CacheService` | Target: the native command-registry manifest compiled from every `LandoCommandSpec`. Current implementation: the OCLIF adapter's manifest, covering core built-in commands; plugin commands stay in the separate `plugin-command` cache above, not in this manifest | Rebuild only (`bun run codegen`); there is no runtime write path for this artifact |
