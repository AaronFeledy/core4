# PRD: BETA-06 — Scratch apps

## Introduction

Scratch apps (§21 / `spec/19-scratch-apps.md`) are short-lived isolated apps created either by forking an existing app or scratch-starting from a recipe. Beta ships `ScratchAppService`, the `scratch` bootstrap level, the `apps:scratch:*` CLI namespace, fork-mode and scratch-mode lifecycles, `--isolate=full` (content-copy isolation; copy-on-write deferred to post-4.0), scope-bound finalizer cleanup, a registry + orphan-reap pass, and the `--mount-cwd` / `--share-global-storage` flags.

Depends on: **BETA-05** (global app must be running and reachable for scratch apps to share storage / network).
...
- [`spec/05-runtime-providers.md`](../05-runtime-providers.md) `ProviderCapabilities.copyOnWriteAppRoot` (not used in Beta — content copy only).
...
- [ ] Provider declares `copyOnWriteAppRoot: false` for Beta; reflink/clonefile/overlay paths are preserved as the post-4.0 substitution point.
- [ ] Tests pass; typecheck passes; lint passes.
