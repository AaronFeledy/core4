...
| Service tags | `@lando/core/services` | `ConfigService`, `PathsService`, `LandofileService`, `PluginRegistry`, `CommandRegistry`, `ConfigTranslatorRegistry`, `TemplateEngineRegistry`, `TemplateRenderer`, `RuntimeProviderRegistry`, `AppPlanner`, `EventService`, `CacheService`, `StateStore`, `ManagedFileService`, `FileSystem`, `ProcessRunner`, `ShellRunner`, `BunSelfRunner`, `HttpClient`, `Downloader`, `PrivilegeService`, `EmbeddedAssetService`, `Logger`, `Renderer`, `InteractionService`, `RedactionService`, `Telemetry`, `DeprecationService`, `DoctorService`, `HostProxyService`, `TunnelService`, `McpService`, plus pluggable abstraction tags (§4.2) |
...
| ~~OCLIF adapter~~ | ~~`@lando/core/oclif`~~ | **Removed (architecture-simplicity).** Hosts MUST NOT import an OCLIF adapter; use `@lando/core/cli` or library APIs. |
...
- `@lando/core/oclif` is **removed** from the shipping public/internal export surface (architecture-simplicity US-526). Embedding hosts MUST NOT import it. Tests enforce the boundary. The native dispatcher and `@lando/core/cli` replace any prior OCLIF-adapter need.
