...
export const mergeLogSources = (
  ...
  const userError = validateUniqueSources(input, input.userSources, issuePath(input.serviceName));
  if (userError !== undefined) return Either.left(userError);

  if (input.base !== "lando") {
    const redirect = input.typeSources
      .map((source) => Schema.decodeUnknownSync(LogSource)(source))
      .find((source) => source.strategy === "redirect");
    if (redirect !== undefined) {
      return Either.left(
        validationError(
          input,
          `Service ${input.serviceName} log source ${String(redirect.id)} uses strategy: redirect, but base: ${input.base} does not give Lando a build phase to redirect daemon logs. Use strategy: follow for BYO services.`,
          typeIssue,
        ),
      );
    }
  ...
  for (const source of input.typeSources.map((entry) => Schema.decodeUnknownSync(LogSource)(entry))) {
    merged.set(String(source.id), source);
  }
  for (const source of input.userSources.map((entry) => Schema.decodeUnknownSync(LogSource)(entry))) {
    merged.set(String(source.id), source);
  }
