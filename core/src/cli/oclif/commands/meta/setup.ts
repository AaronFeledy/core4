...
/**
 * Built-in `lando setup` flags. Provider-specific flags (e.g. the Lando runtime
 * bundle overrides) are contributed by plugins through `setup.flags` and merged
 * in below, not hardcoded here.
 */
const SETUP_BUILTIN_FLAGS = {
...
const SETUP_PLUGIN_FLAGS = mergeSetupPluginFlags(SETUP_RESERVED_FLAG_NAMES, BUNDLED_SETUP_FLAG_CONTRIBUTIONS);
...
const contributedSetupFlagsForProvider = (input: unknown, providerId: string): Record<string, unknown> => {
