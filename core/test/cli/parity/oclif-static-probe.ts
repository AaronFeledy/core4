/**
 * OCLIF-in-compiled-binary experiment arm.
 *
 * This standalone entrypoint attempts to get `@oclif/core`'s `execute()` to
 * dispatch a command WITHOUT an on-disk `commands/` tree, the way it would have
 * to inside a `bun build --compile` single-file binary.
 *
 ...
 * the captured error text on stderr) is the behavioral evidence. It prints a
...
try {
  ...
  // If we get here, OCLIF dispatched without on-disk discovery, so the
  // compiled-dispatch conclusion must be revisited.
