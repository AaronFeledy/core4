...
import { AppConfigResultSchema, appConfig } from "../../src/cli/commands/app-config.ts";

test("keeps a user profile secret out of config, disk caches, events, and build transcripts", async () => {
  ...
  const sentinel = "canary-956-f6d042c1-opaque-resolved-value";
  ...
  const provider: RuntimeProviderShape = {
    ...
    execStream: () => {
      ...
      const split = output.indexOf(sentinel) + 12;
      return Stream.make(
  ...
  try {
    ...
    const result = await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          return { view, get, plan, cachedPlan, buildIdentity, events: [...(yield* Queue.takeAll(queue))] };
        }),
    ...
    const configOutput = JSON.stringify([
      Schema.encodeSync(AppConfigResultSchema)(result.view),
      Schema.encodeSync(AppConfigResultSchema)(result.get),
    ]);
    expect(configOutput).toContain(reference);
    expect(configOutput).not.toContain(sentinel);
    ...
    for (const path of cachePaths) expect((await readFile(path)).includes(Buffer.from(sentinel))).toBe(false);
    const artifacts = (
      await Promise.all(
