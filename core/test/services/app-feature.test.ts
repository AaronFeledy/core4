...
const draft = (over: Partial<AppFeatureServiceDraft> & { serviceName: string }): AppFeatureServiceDraft => ({
  ...
  ...(over.framework === undefined ? {} : { framework: over.framework }),
  ...
  ...(over.command === undefined ? {} : { command: over.command }),
  ...(over.entrypoint === undefined ? {} : { entrypoint: over.entrypoint }),
  environment: over.environment ?? {},
  ...(over.user === undefined ? {} : { user: over.user }),
  ...(over.workingDirectory === undefined ? {} : { workingDirectory: over.workingDirectory }),
  ...(over.appMount === undefined ? {} : { appMount: over.appMount }),
  mounts: over.mounts ?? [],
  buildSteps: over.buildSteps ?? [],
  storage: over.storage ?? [],
  endpoints: over.endpoints ?? [],
  dependsOn: over.dependsOn ?? [],
  ...(over.healthcheck === undefined ? {} : { healthcheck: over.healthcheck }),
  ...(over.certs === undefined ? {} : { certs: over.certs }),
