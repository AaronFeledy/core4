...
const workflowPath = resolve(repoRoot, ".github/workflows/rails-journey.yml");
...
const JOURNEY = "bun run scripts/rails-journey.ts";
...
const jobBlock = (workflow: string, id: (typeof PLATFORM_IDS)[number]): string => {
  ...
  return findIndentedBlock(jobs, `rails-journey-${id}`, 2);
};

describe("rails-journey workflow", () => {
  test("starts with the generated-file header and six platform jobs", async () => {
    ...
    for (const id of PLATFORM_IDS) {
      expect(jobs).toContain(`  rails-journey-${id}:`);
    }
  });
  ...
  test("runs setup before the Rails journey on every cell", async () => {
    ...
    for (const id of PLATFORM_IDS) {
      ...
      const journey = job.indexOf("Run Rails journey");
  ...
  test("does not set COMPOSER_PROCESS_TIMEOUT", async () => {
    // Given / When
    const workflow = await readWorkflow();

    // Then
    expect(workflow).not.toContain("COMPOSER_PROCESS_TIMEOUT");
  });
  ...
  test("names the linux-x64 evidence artifact after the job id", async () => {
    ...
    // Then
    expect(linuxX64).toContain("name: rails-journey-linux-x64");
  });

  test("passes --report to the Rails journey script", async () => {
    // Given / When
    const linuxX64 = jobBlock(await readWorkflow(), "linux-x64");

    // Then
    expect(linuxX64).toContain("--report");
    expect(linuxX64).toContain("scripts/rails-journey.ts");
  });
