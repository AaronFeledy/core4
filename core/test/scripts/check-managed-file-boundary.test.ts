...
describe("managed-file boundary lint gate", () => {
  test("passes when ownership-marker logic is confined to managed-file/src and tests", async () => {
    const root = await makeFixtureRoot();
    try {
      await write(root, "managed-file/src/marker.ts", 'const tag = "lando-generated";\n');
      await write(root, "managed-file/src/fence.ts", 'const open = ">>> lando:";\n');
      await write(root, "core/src/cli/commands/init.ts", "await service.apply(files);\n");
      await write(root, "managed-file/src/service.test.ts", 'expect(c).toContain("lando-generated");\n');
