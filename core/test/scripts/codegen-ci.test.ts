...
const platformReadinessWorkflowPath = resolve(repoRoot, ".github/workflows/platform-readiness.yml");
const drupalJourneyWorkflowPath = resolve(repoRoot, ".github/workflows/drupal-journey.yml");
const railsJourneyWorkflowPath = resolve(repoRoot, ".github/workflows/rails-journey.yml");
const workflowPerformanceWorkflowPath = resolve(repoRoot, ".github/workflows/workflow-performance.yml");
...
const runtimeBundleWorkflowGeneratorPath = resolve(repoRoot, "scripts/build-runtime-bundle-workflow.ts");
...
describe("ci workflow codegen", () => {
  ...
  test(
    "regenerates runtime-bundle.yml with provider matrix dispatch after manifest repins",
    async () => {
      await runCodegen();

      const firstWorkflow = await readFile(runtimeBundleWorkflowPath, "utf8");
      expect(firstWorkflow.startsWith(`${generatedRuntimeBundleHeader}\n`)).toBe(true);
      expect(firstWorkflow).toContain("    permissions:\n      contents: write\n      actions: write");
      expect(firstWorkflow).toContain("      - name: Commit recovered manifest pin");
      expect(firstWorkflow).toContain("        id: manifest-pin");
      expect(firstWorkflow).toContain("          git push origin HEAD:main");
      expect(firstWorkflow).toContain('          echo "pushed=true" >> "$GITHUB_OUTPUT"');
      expect(firstWorkflow).toContain("      - name: Dispatch provider matrix after manifest repin");
      expect(firstWorkflow).toContain("        if: steps.manifest-pin.outputs.pushed == 'true'");
      expect(firstWorkflow).toContain(
        '        run: GITHUB_TOKEN="${{ github.token }}" gh workflow run provider-matrix.yml --ref main',
      );
      expect(firstWorkflow.indexOf("git push origin HEAD:main")).toBeLessThan(
        firstWorkflow.indexOf("Dispatch provider matrix after manifest repin"),
      );

      await runCodegen();

      expect(await readFile(runtimeBundleWorkflowPath, "utf8")).toBe(firstWorkflow);
    },
    codegenTestTimeout,
  );
  ...
  test(
    ...
    async () => {
      ...
      const runtimeBundleWorkflow = await readFile(runtimeBundleWorkflowPath, "utf8");
      ...
      expect(runtimeBundleWorkflow).not.toContain("bun-version: ");

      for (const generatedWorkflow of [
        workflow,
        nightlyWorkflow,
        releaseWorkflow,
      ...
      for (const generator of [
        ...
        runtimeBundleWorkflowGenerator,
      ]) {
      ...
      try {
        ...
        const updatedRuntimeBundleWorkflow = await readFile(runtimeBundleWorkflowPath, "utf8");
        ...
        expect(changedLines(runtimeBundleWorkflow, updatedRuntimeBundleWorkflow)).toEqual([]);
      } finally {
  ...
  test(
    "generates platform-readiness.yml with six platform-readiness job keys",
    async () => {
      await runCodegen();

      const workflow = await readFile(platformReadinessWorkflowPath, "utf8");
      expect(workflow.startsWith(`${generatedPlatformReadinessHeader}\n`)).toBe(true);
      for (const id of [
        "linux-x64",
        "linux-arm64",
        "darwin-arm64",
        ...
        expect(workflow).toContain(`platform-readiness-${id}:`);
      }
    },
    codegenTestTimeout,
  );
  ...
  test(
    "generates drupal-journey.yml with six drupal-journey job keys",
    async () => {
      await runCodegen();

      const workflow = await readFile(drupalJourneyWorkflowPath, "utf8");
      expect(workflow.startsWith(`${generatedDrupalJourneyHeader}\n`)).toBe(true);
      for (const id of [
        "linux-x64",
        "linux-arm64",
        "darwin-arm64",
        ...
        expect(workflow).toContain(`drupal-journey-${id}:`);
      }
    },
    codegenTestTimeout,
  );
  ...
  test(
    "generates rails-journey.yml with six rails-journey job keys",
    async () => {
      await runCodegen();

      const workflow = await readFile(railsJourneyWorkflowPath, "utf8");
      expect(workflow.startsWith(`${generatedRailsJourneyHeader}\n`)).toBe(true);
      for (const id of [
        "linux-x64",
        "linux-arm64",
        "darwin-arm64",
        ...
        expect(workflow).toContain(`rails-journey-${id}:`);
      }
    },
    codegenTestTimeout,
  );
  ...
  test(
    "generates workflow-performance.yml from the two Linux managed-runtime readiness cells",
    async () => {
      await runCodegen();
