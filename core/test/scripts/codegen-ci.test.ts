...
describe("ci workflow codegen", () => {
  ...
  test(
    "generates npm beta package publishing with dry-run coverage and dev tag retirement",
    async () => {
      await runCodegen();
      ...
      expect(workflow).toContain("npm-beta-packages:");
      ...
      expect(workflow).toContain("LANDO_NPM_VERSION: 4.0.0-beta.${{ github.run_number }}");
      expect(workflow).toContain("run: bun run scripts/prepare-npm-dev-packages.ts");
      expect(workflow).toContain("npm publish --workspace @lando/sdk --dry-run --access public --tag next");
      expect(workflow).toContain("npm publish --workspace @lando/core --dry-run --access public --tag next");
      expect(workflow).toContain(
        "npm publish --workspace @lando/service-lando --dry-run --access public --tag next",
      );
      expect(workflow).toContain("Retire npm dev dist-tags");
      expect(workflow).toContain("npm dist-tag rm @lando/core dev 2>/dev/null || true");
      expect(workflow).toContain("npm dist-tag rm @lando/service-lando dev 2>/dev/null || true");
      expect(workflow).toContain(
        "npm publish --workspace @lando/sdk --access public --tag next --provenance",
      );
      expect(workflow).toContain(
        "npm publish --workspace @lando/core --access public --tag next --provenance",
      );
      expect(workflow).toContain('test "$before_latest" = "$after_latest"');
      expect(workflow).toContain("npm view @lando/core dist-tags.next --json");
      expect(workflow).toContain(`grep -Eq '"?4\\.0\\.0-beta\\.[0-9]+"?'`);
