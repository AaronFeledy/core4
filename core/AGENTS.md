...
- MCP projections for a mixed read/write umbrella command must replace the umbrella with constrained read specs and omit its separately registered write-verb ids from the MCP registry. Merely leaving the umbrella out of the default allowlist is insufficient because explicit `mcp.allow` can otherwise reach those compiled verb ids.
