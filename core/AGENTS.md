...
- `@lando/sdk/services` publishes the public `HttpClient` tag with the same `@lando/core/HttpClient` key. `core/src/http-client/service.ts` remains the provisional stream-only core implementation until a follow-on change promotes `HttpClientLive`, the canonical network-trust resolver, and the network-boundary gate.
