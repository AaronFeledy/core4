...
The renderer rule is classified `thin` after removing two obsolete source carve-outs. Paths, probe, redaction, network, and state-store already contain only residual enforcement. The sole `delete` verdict has been executed. The `@lando/redaction` and `@lando/http-client` package extractions paid the scanner-retirement ratchet by converting the redaction and network rules to owner-excluding scopes and deleting the network rule's `live.ts` file carve-out.
...
| **managed-file** | keep | `grep -R -n -m 3 -e lando-generated -e '>>> lando:' -e '<<< lando:' engine/src/managed-file --include='*.ts'` found the marker owner residue, while extracted engine operations share its package and therefore cannot be constrained by a package edge. |
...
| **spec-reference** | keep | `bun run scripts/check-boundaries.ts spec-reference` exercised the repository-wide content scan and reported only the known untracked `.omo/**` planning noise; references to the removable specification tree remain content rather than package edges. |
