...
The renderer rule is classified `keep` after the `@lando/renderer` extraction converted it to an owner-excluding residual rule, removed the compiled-shell carve-out, added `core/bin` coverage, and strengthened detection from call expressions to property-access-based calls, aliases, and assignments involving console/process writes. Paths, probe, redaction, network, managed-file, and state-store already contain only residual enforcement. The sole `delete` verdict has been executed. The `@lando/redaction`, `@lando/http-client`, `@lando/managed-file`, and `@lando/renderer` package extractions paid the scanner-retirement ratchet by converting residual rules to owner-excluding scopes or deleting obsolete owner coverage. The `@lando/mcp` extraction paid by eliminating the machine-output rule's final carve-out (one to zero) after canonical serialization moved behind `@lando/sdk/command-result`, making the rule stronger. The `@lando/data-mover` and `@lando/telemetry` extractions pay the same ratchet by removing their former directories from the engine-closure structural scan; package-DAG now owns those package boundaries. The `@lando/container-runtime` Podman seam (neutral libpod operations plus the single `LIBPOD_API_PREFIX` owner in `container-runtime/src/podman/api-client.ts`) paid the ratchet by deleting the `libpod-prefix` rule outright: the Podman API version literal now lives in exactly one owned file, pinned by `container-runtime/test/podman-api-client.test.ts`, so a plugin-tree literal scan no longer guards anything.
...
| `package-dag` | `structural` | Undeclared manifest edges, forbidden cross-package source edges, detached test-tier entrypoint edges (cross-package test imports must use the target package's root export or a named, non-wildcard exports subpath), and workspace packages with `src/` but no `test/` tree | All workspace runtime and dev/test edges | `keep` | This is the primary package ownership gate; the @lando/data-mover, @lando/telemetry, and @lando/mcp seams remove former in-package ownership checks while package-DAG enforces the new package directions. |
...
| `renderer` | `behavioral` | Property-access-based calls, aliases, or assignments involving `console.*`, `process.stdout.write`, or `process.stderr.write` outside `@lando/renderer`, including `core/bin` | `@lando/renderer` owns terminal writes; package-DAG owns direction | `keep` | The @lando/renderer seam owns terminal writes; the owner-excluding rule scans runtime consumers plus core/bin for property-access-based console/process write access because package-DAG cannot enforce call-site output routing. Computed access such as `console["log"]` is not detected. |
...
**Deferred follow-up — test-tier direction:** Design-time measurement found `sdk/test` → `@lando/core` 33 times, `container-runtime/test` → `@lando/core` 11 times, `engine/test` → `@lando/core` 4 times, plus plugin cross-edges. The direction check remains deferred because `sdk/test/library` intentionally exercises the library API in a dedicated CI job.
...
| **renderer** | keep | `bun run scripts/check-boundaries.ts renderer` scans the shared runtime tier except `renderer/src`, plus `core/bin`, with no carve-outs. Terminal writes and stdio detachment now live in `@lando/renderer/io`; the rule reports property-access-based call, alias, and assignment references to `console.*` and `process.std*.write` everywhere else, but not computed access such as `console["log"]`. |
