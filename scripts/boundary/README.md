...
| `renderer` | `behavioral` | Direct or aliased `console.*`, `process.stdout.write`, or `process.stderr.write` references outside `@lando/renderer`, including `core/bin` | `@lando/renderer` owns terminal writes; package-DAG owns direction | `keep` | The @lando/renderer seam owns terminal writes; the owner-excluding rule scans all runtime consumers plus core/bin for direct or aliased console/process write access because package-DAG cannot enforce call-site output routing. |
...
| **renderer** | keep | `bun run scripts/check-boundaries.ts renderer` scans the shared runtime tier except `renderer/src`, plus `core/bin`, with no carve-outs. Terminal writes and stdio detachment now live in `@lando/renderer/io`; the rule reports call, alias, and assignment references to `console.*` and `process.std*.write` everywhere else. |
