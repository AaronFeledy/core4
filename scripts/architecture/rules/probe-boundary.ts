...
): ReadonlyArray<ProbeBoundaryMatch> => {
  const offenders: ProbeBoundaryMatch[] = [];
...
export const probeBoundaryRule: Rule = {
  id: "probe-boundary",
  title: "Probe boundary",
  failureHeadline:
  ...
  async run(context): Promise<ReadonlyArray<Diagnostic>> {
    const inventoryFiles = await context.files("core-and-plugin-sources");
    const files = inventoryFiles.map(({ absolutePath }) => absolutePath);
    const sources = new Map<string, ts.SourceFile>();
    await Promise.all(
      inventoryFiles.map(async (file) => {
        sources.set(file.absolutePath, await context.sourceFile(file));
      }),
    );
    ...
    return inventoryFiles
      .flatMap((file) => {
        ...
        const bindings = moduleCache.get(file.absolutePath) ?? emptyModuleProbeBindings();
        return scanFileWithBindings(file.relativePath, source, bindings);
      })
