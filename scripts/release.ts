...
export const RELEASE_STAGES: ReadonlyArray<ReleaseStage> = [
  ...
  defineStage({
    id: "7-compile",
    label: "Compile",
    description: "bun build --compile --bytecode --target=bun-windows-x64 bin/lando.ts.",
    forBinary: true,
    forLibrary: false,
    kind: "spawn",
    commandSummary: "bun build ./core/bin/lando.ts --compile --bytecode --target=bun-windows-x64",
    remediation: defaultRemediation,
    command: [
      [
        "bun",
      ...
      ["bun", "run", "scripts/sanitize-compiled-binary.ts", WINDOWS_RELEASE_BINARY],
    ],
  }),
  defineStage({
    id: "8-strip",
