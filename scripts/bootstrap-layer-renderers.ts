export const renderMinimal = (): string =>
  [
    ...
    'import { CacheServiceWithPrivateFileAccessLive } from "@lando/engine/cache/service";',
    ...
    'import { ManagedFileServicesWithPrivateFileAccessLive } from "@lando/managed-file/service";',
    'import { ManagedFileTransactionGuardWithPrivateFileAccessLive } from "@lando/managed-file/transaction";',
    ...
    'import { makeSecretStoreRegistryLive, RoutedSecretStoreLive } from "@lando/engine/services/secret-store-registry";',
    'import { StateStoreWithPrivateFileAccessLive } from "@lando/state-store/service";',
    ...
    'import { makeNoneBootstrapLayer } from "./none.ts";',
    "",
    "export const makeMinimalBootstrapLayer = (inputs: BootstrapLayerInputs) => {",
    "  const noneRuntimeLive = makeNoneBootstrapLayer(inputs);",
    ...
    "  const secretStoreLive = RoutedSecretStoreLive.pipe(Layer.provide(Layer.mergeAll(",
    "    makeSecretStoreRegistryLive(bundledPluginModules()), ConfigServiceLive, ProcessRunnerLive, FileSystemLive, pathsLive,",
    "  )));",
    "  const redactionLive = RedactionServiceLive.pipe(Layer.provide(secretStoreLive));",
    "  const privateFileAccessLive = PrivateFileAccessLive;",
    ...
    "    noneRuntimeLive,",
    "    LoggerLive({ mode: inputs.loggerMode, logLevel: inputs.logLevel, structured: inputs.structured, ...(inputs.writeDiagnosticLine === undefined ? {} : { writeLine: inputs.writeDiagnosticLine }), ...(inputs.stderrIsTTY === undefined ? {} : { stderrIsTTY: inputs.stderrIsTTY }) }),",
    ...
    "    CacheServiceWithPrivateFileAccessLive.pipe(Layer.provide(privateFileAccessLive)),",
    "    FileSystemLive,",
    "    ProcessRunnerLive,",
    ...
    "    redactionLive,",
    "    Layer.suspend(() => ManagedFileServicesWithPrivateFileAccessLive).pipe(",
    "      Layer.provide(Layer.mergeAll(eventServiceLive, redactionLive, privateFileAccessLive)),",
    "    ),",
...
export const renderCommands = (): string =>
  [
    ...
    "  const landofileServiceLive = Layer.unwrapEffect(Effect.map(PathsService, (paths) => {",
    "    const runtimeInputs = landofileRuntimeInputs();",
    "    return makeEngineLandofileServiceLive({",
    "      ...runtimeInputs,",
    "      ports: {",
    "        ...runtimeInputs.ports,",
    "        resolveUserIncludesDir: () => paths.userIncludesDir,",
    "        resolveUserCacheRoot: () => paths.roots.userCacheRoot,",
    "      },",
    "    });",
    "  })).pipe(Layer.provide(pluginsBase));",
    ...
    "  const subscriberRuntimeLive = makeSubscriberRuntimeWithPrivateFileAccessLive(bundledPluginModules(), BUILT_IN_COMMAND_IDS).pipe(Layer.provide(commandsBase));",
...
export const renderProvider = (): string =>
  [
    ...
    'import { makeSubscriberRuntimeWithPrivateFileAccessLive } from "@lando/engine/lifecycle/subscribers";',
    'import { makeRuntimeProviderRegistryWithPrivateFileAccess } from "@lando/engine/providers/registry";',
    ...
    "  const providerRegistryLive = makeRuntimeProviderRegistryWithPrivateFileAccess(bundledPluginModules()).pipe(",
    ...
    "    Layer.provide(Layer.mergeAll(baseProviderLayer, globalAppRuntimeLive, routerRegistryLive)),",
    "    Layer.mapError((cause) =>",
    '      new LandoRuntimeBootstrapError({ message: cause instanceof Error ? cause.message : "RouterService bootstrap failed.", stage: "provider", cause }),',
    "    ),",
    "  );",
    ...
    "  const subscriberRuntimeLive = makeSubscriberRuntimeWithPrivateFileAccessLive(bundledPluginModules(), BUILT_IN_COMMAND_IDS).pipe(Layer.provide(providerBase));",
...
export const renderIndex = (): string =>
  [
    ...
    "  switch (bootstrap) {",
    '    case "none":',
    "      return makeNoneBootstrapLayer(inputs);",
    '    case "minimal":',
