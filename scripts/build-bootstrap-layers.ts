...
const renderPlugins = (): string =>
  [
    ...
    "    cwd: inputs.cwd,",
    "  }, BUNDLED_PLUGIN_MODULES).pipe(Layer.provide(minimalRuntimeLive));",
    "  const pluginRegistryLive = makePluginRegistryLive(inputs.pluginDiscovery, BUNDLED_PLUGIN_MODULES).pipe(",
...
const renderTooling = (): string =>
  [
    ...
    "  const subscriberRuntimeLive = makeSubscriberRuntimeLive(BUNDLED_PLUGIN_MODULES, BUILT_IN_COMMAND_IDS).pipe(Layer.provide(toolingBase));",
...
const renderScratch = (): string =>
  [
    ...
    "  const proxyRegistryLive = makeProxyServiceRegistryLive(BUNDLED_PLUGIN_MODULES).pipe(",
...
const renderApp = (): string =>
  [
    ...
    "    makeBundledFileSyncEngineLive(BUNDLED_PLUGIN_MODULES).pipe(Layer.provide(providerBase)),",
    "  );",
    "  const proxyRegistryLive = makeProxyServiceRegistryLive(BUNDLED_PLUGIN_MODULES).pipe(",
    ...
    "  const subscriberRuntimeLive = makeSubscriberRuntimeLive(BUNDLED_PLUGIN_MODULES, BUILT_IN_COMMAND_IDS).pipe(Layer.provide(fullAppBase));",
