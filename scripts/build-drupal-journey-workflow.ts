...
const OUTPUT = resolve(REPO_ROOT, ".github/workflows/drupal-journey.yml");
...
const assertNever = (value: never): never => {
  throw new Error(`Unexpected drupal-journey value: ${String(value)}`);
};
...
${renderSetupStep(cell, binary)}
      ...
      - name: Run Drupal journey
        run: bun run scripts/drupal-journey.ts --binary ${binary} --app-dir \${{ runner.temp }}/drupal-app --report \${{ runner.temp }}/drupal-journey.json
      ...
      - name: Upload evidence
        ...
        with:
          name: drupal-journey-${cell.id}
          path: \${{ runner.temp }}/drupal-journey.json
          if-no-files-found: ignore
          retention-days: 14
`;
};

export const renderDrupalJourneyWorkflow = (): string => `${GENERATED_HEADER}
name: drupal-journey
...
const main = async (): Promise<void> => {
  await Bun.write(OUTPUT, renderDrupalJourneyWorkflow());
  console.log(`[build-drupal-journey-workflow] wrote ${OUTPUT}`);
};
