...
Failure policy follows consent. Sidecar mode is best-effort: a missing capability, sidecar, or relay logs a warning, the app starts without the overlay, and doctor reports degraded. Host mode fails closed with `SshAgentUnavailableError` (`host-agent-not-found`, `socket-missing`, `capability-missing`, `sidecar-not-running`, `bridge-failed`) or `SshAgentTransportError` (`broker`, `worker`, `bridge`), each with remediation. The planner records only eligible services and the selected mode; the socket path and mount are a start-time overlay that never enters the plan cache. `lando doctor` states the security implication of host mode and never prints agent contents or claims a key moved into a container.
