...
The matrix at `docs/reference/compose-key-matrix.mdx` is generated by `scripts/build-compose-key-matrix.ts` from the committed disposition matrix in `core/src/landofile/compose/dispositions.ts`, and CI drift-gates it with `bun run codegen:compose-key-matrix` followed by `git diff --exit-code`. Documentation, JSON Schema, and `app:config:lint` are derived from that one source and cannot diverge from it.
