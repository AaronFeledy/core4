...
Local fast path: `bun run scripts/check-boundaries.ts --all` runs every boundary rule in one parse pass. Individual `bun run check:<gate>` commands remain the CI contract; each is a thin shim over the declarative rules in `scripts/boundary/rules/`.
