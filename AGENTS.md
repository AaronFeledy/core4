...
- User-app resolution should go through `loadUserLandofile(...)` from `core/src/landofile/app-resolution.ts`; CLI modules may use the CLI-only re-export at `core/src/cli/app-resolution.ts`. Do not call raw `LandofileService.discover`.
