...
- Also run `bun run lint` (which already includes `check:architecture`, the architecture boundary kernel covering import-cycle, managed-file-boundary, package-dag, probe-boundary, redaction-boundary, renderer-boundary, and state-store-boundary; narrow a single rule with `--rule=<id>`) plus any touched codegen/guide gate: `check:guide-coverage`, `check:guide-drift`, `check:public-transcripts`, `check:telemetry-inventory`, or `lint:guides`.
...
- Plugins may not depend on `@lando/core`; core imports plugin packages only via the generated composition root (`core/src/plugins/generated/**`), enforced by `check:architecture --rule=package-dag` (the `check:package-dag` wrapper still works).
...
- Use the Effect-free paths primitive in `core/src/config/paths.ts` for Lando roots and derived paths; do not re-spell `$HOME`, XDG, `%APPDATA%`, or platform separators. Hand-rolled `join(<userDataRoot>, "plugins"|"bin")` / `join(<userCacheRoot>, "scratch")` is blocked by `check:architecture --rule=paths-boundary` (the `check:paths-boundary` wrapper still works); route through `makeLandoPaths` (pure) or `PathsService` (Effect). A genuinely host-bound path that must ignore a faked `process.platform` (e.g. mutagen install dirs) should pin `makeLandoPaths({ platform: sep === "\\" ? "win32" : "linux" })` from `node:path.sep`, not read `process.platform`.
- Durable atomic, versioned, lockable state belongs in `StateStore` (`core/src/state/**`); plugins use `LandoPluginContext.stateStore`; host/tests override `StateStore` or use `TestStateStore`. Do not hand-roll write-temp+rename+lockfile+version envelopes; `check:architecture --rule=state-store-boundary` enforces this (the `check:state-store-boundary` wrapper still works).
- Host/provider-shaped retry/backoff/timeout-to-verdict probing (healthcheck, scanner, doctor, downloader, setup readiness) must build on `@lando/sdk/probe`'s `runProbe`; net-new hand-rolled `Effect.retry`/`Effect.repeat`/`Effect.schedule`/`Schedule.*` loops in `core/src/**` or `plugins/**` are blocked by `check:architecture --rule=probe-boundary` (the `check:probe-boundary` wrapper still works; allowlist the advisory-lock loop in `core/src/state/lock.ts`). Consumers redact `ProbeResult.lastError` through `RedactionService` before it reaches an event, transcript, or readiness summary.
