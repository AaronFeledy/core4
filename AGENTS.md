...
- Command output goes through the `Renderer` service. `@lando/renderer` owns terminal writes; direct or aliased `console.*` or `process.std*.write` references under the rest of the shared shipped-runtime tier and `core/bin/**` fail the owner-excluding renderer-boundary gate, with no file carve-outs.
