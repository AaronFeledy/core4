...
- Also run `bun run lint` and any touched boundary/codegen/guide gate: `check:renderer-boundary`, `check:managed-file-boundary`, `check:redaction-boundary`, `check:telemetry-inventory`, `lint:guides`, `check:guide-coverage`, `check:public-transcripts`, or `check:guide-drift`.
