...
- Tests live in the package that owns the code under test; cross-package test imports use the target's root export or a named exports subpath (wildcard subpaths do not count), and every workspace package with `src/` ships tests under `<pkg>/test/`; enforced by the `package-dag` test tier in `check:boundaries`.
