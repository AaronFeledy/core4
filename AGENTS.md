...
- Lando v4 is a pre-release Bun monorepo. Workspace packages: `@lando/core` (CLI + library API + wiring), `@lando/sdk` (public contracts), `@lando/engine` (orchestration runtime), `@lando/container-runtime`, and the private packages `@lando/paths`, `@lando/state-store`, `@lando/landofile`, `@lando/redaction`, `@lando/http-client`, `@lando/managed-file`, `@lando/data-mover`, `@lando/telemetry`, `@lando/renderer`, `@lando/mcp`, plus bundled plugins under `plugins/*`.
...
- Each plugin-abstraction contract suite from `@lando/sdk/test` must stay listed in `core/test/contract/plugin-abstraction-coverage.test.ts` and exercised by its documented built-in invocation in the owning package's test tree (`core/test/**` or `engine/test/**`) unless the abstraction's pluggability-catalog entry documents that no built-in ships (e.g. `InitSource`, `ServiceFeature`, `AppFeature`, and `ConfigTranslator` are plugin-only or have no bundled default).
...
- `@lando/engine` ships an explicit exports map (no wildcard); every externally-consumed subpath must be listed there.
- `@lando/redaction` owns the canonical `RedactionService`/standalone redactor; `@lando/http-client` owns HTTP egress (`HttpClientLive`, network trust, downloader) and is the only package allowed to call global `fetch`; `@lando/managed-file` owns `ManagedFileService` Live, ownership markers, codecs, overwrite decisions, and the private multi-file transaction coordinator on the `@lando/managed-file/transaction` subpath. Recoverable multi-file edits go through that coordinator instead of hand-rolled staging: it takes a canonical-app-root advisory lock, rejects symlinked targets and parents outside that root, writes same-directory `<target>.lando-stage.<id>` files plus immutable `<file>.bak.<full-before-sha256>` backups, then fsyncs an owner-only `prepared` journal (under the state-store user-data root, the one exception to app-root containment) before any mutation and steps it through `committing` and `committed`. Journals, errors, and receipts carry digests, modes, and paths only, never file bytes.
- `@lando/mcp` owns MCP catalog projection, JSON-RPC transport, and retained-runtime dispatch. It consumes core command execution only through `McpCommandExecutor`; core implements that port with its command lifecycle and injects it at composition.
...
- Tests live in the package that owns the code under test; cross-package test imports use the target's root export or a named exports subpath (wildcard subpaths do not count), follow the package DAG (a package's tests import only its `dependencies`/`devDependencies` or a commented `testTargets` override in `scripts/boundary/rules/package-dag-policy.ts`), and every workspace package with `src/` ships tests under `<pkg>/test/`; enforced by the `package-dag` test tier in `check:boundaries`. Plugin tests never import `@lando/core`: engine-owned doubles live on `@lando/engine/testing/*` (`state-store`, `downloader`, `live-provider-socket`), Live layers on their `@lando/engine/*` subpaths, and renderer IO on `@lando/renderer/*`; `@lando/core/testing` is for core's own tests and embedding hosts.
...
- Command output goes through the `Renderer` service. `@lando/renderer` owns terminal writes; property-access-based calls, aliases, or assignments involving `console.*` or `process.std*.write` under `core/src/**`, `data-mover/src/**`, `engine/src/**`, `http-client/src/**`, `landofile/src/**`, `managed-file/src/**`, `mcp/src/**`, `paths/src/**`, `redaction/src/**`, `state-store/src/**`, `telemetry/src/**`, `plugins/**`, and `core/bin/**` fail the owner-excluding renderer-boundary gate, with no file carve-outs. Computed access such as `console["log"]` is outside this rule's detection.
...
- A versioned service type (`type: mariadb:10.6`) makes the planner write the type's own published artifact tag onto the config as `image` before service-type resolution, so `ServiceConfig.image` inside the planner does not prove the author supplied an image. Compare against the `resolvedArtifactTag` carried on `ResolvedService` before treating a service as custom-imaged.
...
- Intel Mac (`darwin`/`x64`): `@lando/provider-lando` fail-closes via `rejectIntelMacHost` (setup, bundle, ensureRuntime, getStatus) with remediation naming `lando setup --provider=docker` / `LANDO_PROVIDER=docker`. Do not flip the capability default to docker, and do not set `isAvailable` false (file-sync would skip).
