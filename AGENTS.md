...
- For compiled Bun binaries, do not read package metadata with `Bun.file(new URL(...package.json, import.meta.url))`; the compiled artifact resolves that path through Bun's virtual filesystem. Statically import `core/package.json` and include it in `core/tsconfig.json` instead. Also keep the MVP OCLIF binary unminified on Bun 1.2.18 because `--minify` can produce a runtime `ReferenceError` during the dynamic OCLIF import.
