...
`--yes` uses PHP 8.3, MariaDB 11.4, Composer 2, webroot `/app/public`, and no worker. Pass `--answer` to change those. PHP 8.6 is a valid `--answer=php=8.6`. It does not include **xdebug**, **redis**, or **apcu**. See [Pin a PHP version](../../docs/guides/services/php-version-matrix.mdx).
