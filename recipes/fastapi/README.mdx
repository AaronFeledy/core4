...
<Guide>
  ...
  <Scenario id="declared-stack" render={false} reason="Verify framework and Redis defaults without repeating the reader setup">
    <Step name="init">
      <Variable name="stackName" value="fastapi-stack" display="my-fastapi-stack" />
      <Run command="lando init --recipe fastapi --name={{stackName}} --yes" />
    </Step>
    <Step name="inspect">
