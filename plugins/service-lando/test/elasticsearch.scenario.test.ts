...
describe("elasticsearch service type — scenario: Elasticsearch + lando es-cli tooling", () => {
  ...
  test("`lando es-cli /_cluster/health` tooling alias routes through provider.exec to the search service", async () => {
    ...
    const result = await Effect.runPromise(
      runTooling({ name: "es-cli", args: ["/_cluster/health"] }).pipe(Effect.provide(layer)),
    );

    expect(result.tool).toBe("es-cli");
  ...
  test("`lando es-cli /_cat/indices` routes through provider.exec and returns index listing", async () => {
    const landofile = Schema.decodeUnknownSync(LandofileShape)({
      name: "myapp",
      services: { search: { type: "elasticsearch" } },
      tooling: {
    ...
    const result = await Effect.runPromise(
      runTooling({ name: "es-cli", args: ["/_cat/indices"] }).pipe(Effect.provide(layer)),
    );
