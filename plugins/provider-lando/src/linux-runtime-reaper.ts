...
export type { LinuxRuntimeFilesystem } from "./linux-runtime-generation.ts";
...
const terminationPolicy: RetryPolicy = {
  maxAttempts: 61,
...
const reaperError = (message: string, details: object, cause?: unknown) =>
  new ProviderUnavailableError({
    ...
    operation: "setup",
    message,
    remediation: "Stop the managed runtime service, then rerun the command.",
    details,
...
const servicePids = (deps: LinuxRuntimeReaperDeps): Effect.Effect<ReadonlyArray<number>> =>
  Effect.gen(function* () {
    ...
    const alive: number[] = recordedOwned ? [recordedPid] : [];
    for (const pid of candidates) {
      if (recordedOwned && pid === recordedPid) continue;
...
const terminateAndWait = (
  ...
  return Effect.forEach(
    pids,
    (pid) =>
      deps.serviceRunner.terminate(pid, spec).pipe(
        Effect.andThen(
          runProbe(
            {
              id: `provider-lando-runtime-terminate-${pid}`,
              policy: deps.terminationPolicy ?? terminationPolicy,
              classify: {
