...
import { withAdvisoryLock } from "@lando/core/advisory-lock";
...
import { recordedLaunchMatchesSpec, writeLaunchState } from "./runtime-launch-state.ts";
import {
...
const ensureLinuxRuntime = (deps: EnsureRuntimeDeps): Effect.Effect<void, ProviderUnavailableError> =>
  Effect.gen(function* () {
    ...
    yield* runPhase(
      ...
      "launch",
      withAdvisoryLock(
        `${deps.runRoot}.generation`,
        "ensure-lando-runtime",
        Effect.gen(function* () {
          if (yield* currentRuntimeIsOwned(deps)) return;
          yield* reapStaleLinuxRuntime(deps);
        ...
        Effect.mapError((cause) =>
          cause instanceof StateStoreError
            ? new ProviderUnavailableError({
                ...
                details: { lockTarget: `${deps.runRoot}.generation` },
                cause,
              })
            : cause,
        ),
