export type ComposeDisposition = "normalized" | "preserved" | "rejected" | "unknown";

// Local exact-path mirror of landofile/src/compose/dispositions.ts; unknown paths stay unknown.
...
export const dispositionOf = (path: string): ComposeDisposition => {
  if (normalized.has(path)) return "normalized";
  if (preserved.has(path)) return "preserved";
