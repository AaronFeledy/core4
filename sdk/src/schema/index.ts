...
// FileSyncEngine — pluggable file-sync engine contract published from Beta.
// Spec source: §10.6.1 (FileSyncEngine architecture).
...
/** Sync direction + conflict-resolution mode (§10.6.1). */
...
/**
 * Engine capability matrix declared at registration time (§10.6.1). Used by
...
/**
 * Per-mount session creation spec (§10.6.1). Generated by `AppPlanner`
 * from accelerated `MountPlan` entries.
 */
export const FileSyncSessionSpec = Schema.Struct({
  ...
  service: ServiceName,
  /** Stable §6.4 mount key inside the owning service. */
  ...
  mode: FileSyncMode,
  /** Engine-honored exclude patterns (`excludes:` from §6 service config). */
  excludes: Schema.Array(Schema.String),
