...
- A field `description` does not survive `Schema.encodedBoundSchema` when the field is a transform, so a new transformed field passes the public-field-description gate on canonical `ServiceConfig` but fails it on `ServiceConfigInput`. Annotate the canonical field (that side is gate-enforced) and add the input field to `PUBLIC_FIELD_DESCRIPTION_EXEMPTIONS`, which is where every other transformed `ServiceConfigInput` field already lives. The gate only fires during `codegen:schema-snapshot`, so run that early rather than as a final formality.
