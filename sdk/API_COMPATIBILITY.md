...
- `SshAgentConfig.upstream` is an additive optional Landofile and global-config field. `"host"` resolves from the host CLI `$SSH_AUTH_SOCK`; an absolute Unix socket path is validated as a socket path. Unset keeps the sidecar file-load from `~/.ssh`. Apps still use the Lando sidecar socket from `getAgentSocket`. Windows upstream hard-fails with remediation. `@lando/sdk/schema` additively exports `SshAgentUpstream` and `isAbsoluteUnixSocketPath`. `GlobalConfig.sshAgent` and `GlobalConfigView.sshAgent` carry the same knob for the shared sidecar.
