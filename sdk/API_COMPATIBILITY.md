...
- `@lando/sdk/renderer` exports the terminal-renderer contracts `RendererIO` and `RendererContribution` plus the runtime `RendererCapabilities` schema and snapshot constants (`RENDERER_CAPABILITIES_NONE` / `TTY_INITIAL` / `VERBOSE_TTY`). The bundled `@lando/renderer-lando` plugin owns the default `lando` renderer implementation and exports a finished `RendererContribution`; core resolves it through the bundled-renderer registry. Raw OpenTUI probing is never published here.
