...
- `SshAgentConfig.upstream` is an additive optional Landofile and global-config field. `"host"` resolves from the host CLI `$SSH_AUTH_SOCK`; an absolute Unix socket path is validated as a socket path. Authored resolution is env, then global config, then the cwd Landofile. Unset keeps the sidecar file-load from `~/.ssh`. Apps still use the Lando sidecar socket from `getAgentSocket`. Windows upstream hard-fails with remediation. A missing host socket rematerializes the sidecar to file-load. `@lando/sdk/schema` additively exports `SshAgentUpstream` and `isAbsoluteUnixSocketPath`. `GlobalConfig.sshAgent` and `GlobalConfigView.sshAgent` carry the same knob for the shared sidecar.
...
- `@lando/sdk/secrets` is the canonical redaction primitive. In addition to the prior `createSecretRedactor` value layer and the `REDACTED` (`[redacted]`) sentinel, it adds the pure runtime helpers `createRedactor(profile, options)` (returning `{ redactString, redactStringBounded?, redactValue }`), the `REDACTION_PROFILES` tuple (`secrets`/`telemetry`/`transcript`) with its `RedactionProfile` type, and the `PATTERN_CLASSES` catalog. `SecretRedactor.redactBounded?` and `Redactor.redactStringBounded?` accept `(text, maxBytes)` and return `undefined` instead of retaining output beyond the UTF-8 byte budget; consumers that require bounded output fail closed when the method is absent or returns `undefined`. `createRedactor("secrets", ...)` supplies the bounded method; the normalization profiles may omit it. The module stays pure and dependency-free (no `@lando/core`, no Effect runtime, no Node/Bun IO); transcript environment roots are supplied by the caller via `options.env` rather than read from `node:os`. `@lando/core/secrets` re-exports this subpath.  `isUsableExactRedactionValue` rejects empty tokens, short decimal CSI/SGR codes (pure digits of length ≤ 4 such as `0`/`32`/`95`), and semicolon CSI lists (`0;1`, `38;2;255;0;0`) so the value layer cannot splice `[redacted]` into an SGR sequence, while still accepting real secrets including long numeric tokens (`12345678`) and one-character letters (`a`). These are additive Beta helpers and carry no compatibility freeze (the `@lando/sdk/secrets` runtime surface is not part of the frozen schema/service-tag `toEqual` lists below).
...
- `@lando/sdk/task-progress` is the shared task-tree publisher and controller, the same additive-Beta utility tier as `@lando/sdk/probe`. It exports `ProgressEmitter`, the sync publish helpers (`publishTreeStart`, `publishTaskStart`, `publishTaskDetail`, `publishTaskComplete`, `publishTaskFail`, `publishTreeComplete`), `makeTaskTree`, `runWithTaskTree`, and generic `startChildTaskId`. `startTask` additively accepts optional `transcriptPath`; `TaskFailOptions` additively accepts optional `remediation`. These helpers serialize the existing task event schemas; no schema registry membership changes and no frozen `@lando/sdk/errors` widening. The module imports only effect plus type-only sibling events/schema/services, constructs no `LandoRuntime`, and pulls no service Layer. These are additive Beta contracts and carry no compatibility freeze (the `@lando/sdk/task-progress` surface is not part of the frozen schema/service-tag `toEqual` lists below).
- `@lando/sdk/fuzzy` additively exports `rankFuzzy` and `bestFuzzy`. Not a schema/service-tag; same contracts-only tier as `@lando/sdk/expressions` / `@lando/sdk/probe` / `@lando/sdk/secrets` (no Effect runtime, no Bun, no IO).
...
- `SshAgentConfig.upstream`
- `GlobalConfig.sshAgent`
