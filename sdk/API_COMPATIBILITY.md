...
- `NetworkCaConfig.injectIntoServices` (default `true`) and `NetworkProxyConfig.injectIntoServices` (default `false`) are additive optional booleans so host-global `network.ca` / `network.proxy` can drive in-service trust-store and proxy-env install for `type: lando` services without per-project Dockerfiles (§6.8 / §7.5 / §10.3.1). Omitted values decode to the defaults above; prior configs without the keys remain valid.
