...
- US-472 makes `DependencyPlan.condition` use the normative `ServiceDependencyCondition` literals (`service_started`, `service_healthy`, and `service_completed_successfully`) and makes `DependencyPlan.required` a required planner-resolved boolean. `ServiceDependencyCondition` is registered as its own public JSON Schema identifier. `RuntimeProviderShape` additively gains the mandatory scoped `waitForExit(target, options?)` method returning `ServiceExitResult` so orchestration can observe a service process exit code; `WaitForExitOptions.signal` and `BuildAppOptions.signal` propagate cancellation through dependency gates.
