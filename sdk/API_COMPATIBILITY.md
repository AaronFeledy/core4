...
- `RuntimeProviderShape.prepareFileSyncTargets(plan)` is an optional pre-apply hook for providers that can verify accelerated mount targets before app containers start. It returns exact, schema-derived container endpoints for every planned session. Its rollback action is optional because some providers cannot safely delete prepared resources. Startup rejects incomplete, duplicate, or mismatched endpoints before creating a session; when rollback is unavailable after a failed start, it retains the accelerated-start journal and reports manual recovery steps. Providers without the hook use ordinary bind mounts; the bundled managed provider does not yet expose this hook while durable session ownership and safe cleanup are being completed.
