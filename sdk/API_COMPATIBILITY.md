...
- `PluginContribution.configTranslators` is a new additive optional field for contributing `ConfigTranslator` implementations; each `ConfigTranslatorContribution` declares `id`, `module`, and `inputKinds` plus optional `detects`, `optionsSchema`, `summary`, and `deprecated`. `LandoPluginModule.configTranslators` carries matching lazy `ConfigTranslatorLoader` factories, and the additive `ConfigTranslatorRegistry` tag (`@lando/core/ConfigTranslatorRegistry`) resolves them only on explicit conversion; duplicate ids from any plugin source fail with `ConfigTranslatorConflictError` naming both producers.
