...
- `ProxyConfig` additively accepts optional `autoApprove` for invocation-scoped router setup consent. Setup passes it for `--yes` or `--no-interactive`; Traefik uses its existing socket-proxy approval gate. Omitting it preserves interactive consent and any injected router approval policy. This does not authorize or suppress operating-system password prompts.
