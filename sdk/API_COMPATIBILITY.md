...
- `@lando/sdk/plugins` additively exports the `LandoPluginModule` descriptor types, including the `CertificateAuthorityContributionLayer` type and optional `LandoPluginModule.certificateAuthorities` contribution map, and the `definePlugin` helper for packaging a plugin module (manifest, optional contribution maps, optional host-service factories). `PluginStateStore` additively gains `withLock(key, body)` so plugin durable state can take an exclusive lock around a critical section; existing `open` callers are unchanged.
