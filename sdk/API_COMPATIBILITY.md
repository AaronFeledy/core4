...
- Events-as-tasks additively exports the eight-name `AppLifecycleEventName`, strict `EventStep` variants, and `LandofileEvents`; `LandofileShape.events` and `ServiceTypeResolution.events` are optional additions. `@lando/sdk/errors` additively exports `LandofileUnknownEventError`, `LandofileEventStepFailedError`, and `LandofileEventLifecycleReentryError`. `LandofileService.discover`'s error channel additively gains `LandofileUnknownEventError`; the frozen service-surface fixture is updated to match.
